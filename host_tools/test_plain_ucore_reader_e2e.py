...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="plain-ucore-reader-e2e-") as tmp:
        ...
        try:
            ...
            assert any("host_report_workbench_note_title=Scope decision" in line for line in rp_report["lines"]), rp_report
            assert any("host_report_workbench_manifest=delivery-manifest.json" in line for line in rp_report["lines"]), rp_report
            assert any("host_report_workbench_bundle=workbench-bundle.zip" in line for line in rp_report["lines"]), rp_report
            ...
            assert any("host_manifest_workbench=usable-workbench:RUN-E2E" in line for line in rp_manifest["lines"]), rp_manifest
            ...
            assert any("host_manifest_workbench_manifest=delivery-manifest.json" in line for line in rp_manifest["lines"]), rp_manifest
            assert any("host_manifest_workbench_bundle=workbench-bundle.zip" in line for line in rp_manifest["lines"]), rp_manifest
            ...
            assert any("host_action_notebook_workbench_id=usable-workbench:RUN-E2E" in line for line in rp_nbexec["lines"]), rp_nbexec
            ...
            assert any("host_action_workbench_bundle=workbench-bundle.zip" in line for line in rp_uresrun["lines"]), rp_uresrun
