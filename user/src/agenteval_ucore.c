...
static void run_file_query_table_ablation(int load)
{
	...
	rebuild_file_index_diagnostic(experiment, load);
	prepare_file_query_workload(load, 0, EVAL_FILE_QUERIES);
	time_file_table_variant(EVAL_FILE_QUERIES, 0,
				scan_file_observations, &scan);
	time_file_table_variant(EVAL_FILE_QUERIES, 1,
				index_file_observations, &index);
	finalize_agent_file_variant(experiment, load, 0, EVAL_FILE_QUERIES,
	...
	for (int pair = 1; pair <= EVAL_PAIRS; pair++) {
		...
		prepare_file_query_workload(load, pair, EVAL_FILE_QUERIES);
		workload = file_workload_fingerprint(
			experiment, load, pair, EVAL_FILE_QUERIES);
		if (pair_runs_ab(pair)) {
			time_file_table_variant(EVAL_FILE_QUERIES, 0,
						scan_file_observations, &scan);
			time_file_table_variant(EVAL_FILE_QUERIES, 1,
		...
		finalize_agent_file_variant(
			experiment, load, pair, EVAL_FILE_QUERIES, 0, 0,
			scan_file_observations, &scan);
		finalize_agent_file_variant(
			experiment, load, pair, EVAL_FILE_QUERIES, 1, 0,
			index_file_observations, &index);
		check(scan.result_fingerprint == index.result_fingerprint,
		      "metadata table pair equivalence");
		if (pair_runs_ab(pair)) {
			print_sample(experiment, load, pair, "scan", order,
				     "forced-scan", EVAL_FILE_QUERIES, load,
				     workload, &scan);
			print_sample(experiment, load, pair, "index", order,
				     "ready-index", EVAL_FILE_QUERIES, load,
				     workload, &index);
		} else {
			print_sample(experiment, load, pair, "index", order,
				     "ready-index", EVAL_FILE_QUERIES, load,
