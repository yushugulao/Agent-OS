...
static int append_project_lifecycle_actions(void)
{
	...
	if (!rp_append_file("rp_actionio", "host_action_usable_project_outputs=rp_usableproj,rp_usablescaf,rp_usablelaunch,rp_usablepack")) return 0;
	if (!rp_append_file("rp_web_bundle", "host_action_usable_project=rp_usableproj,rp_usablescaf,rp_usablelaunch,rp_usablepack")) return 0;
