...
int main(void)
{
	...
	if (task_lines != 16 || high_tasks != 3 || critical_tasks != 4 || ready_tasks != 16) {
	...
	if (!rp_write_file("rp_rank",
			   ...
			   "critical=4\n"
			   "selected=7\n"
			   "selected_stages=literature,profile,align,decision,measure,risk_capa,release_delta\n"
			   "status=ready\n")) {
		return 1;
	}
	...
	printf("rp_query: workflow=34 agent=26 evidence=10 ranked=16 selected=7 status=ready\n");
	return 0;
}
