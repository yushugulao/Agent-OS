...
int main(void)
{
	...
	if (!rp_write_file("rp_trials",
			   ...
			   "agentos_file_metadata=campaign_trials\n"
			   "status=ready\n")) {
		return 1;
	}
	if (!rp_write_file("rp_camp_rank",
			   ...
			   "agentos_context_record=campaign_ranking\n"
			   "status=ready\n")) {
		return 1;
	}
	...
	if (!rp_append_file("rp_agentcmp", "experiment_campaign_checks=108;campaigns=1;trials=4;best_trial=04;result_review=accept_candidate;kernel_metadata=observed;status=ready")) return 1;
