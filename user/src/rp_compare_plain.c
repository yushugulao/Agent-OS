...
int main(void)
{
	...
	ok = ok && rp_file_contains("rp_dossier", "sections=31");
	...
	ok = ok && rp_file_contains("rp_runview", "scheduler_items=16");
	ok = ok && rp_file_contains("rp_taskrec", "msg=16");
	ok = ok && rp_file_contains("rp_rank", "selected=7");
	ok = ok && rp_file_contains("rp_runview", "ranked_tasks=16");
	...
	ok = ok && rp_file_contains("rp_sched", "queue_items=16");
	...
	ok = ok && rp_file_contains("rp_agentcmp", "message_acks=16");
	ok = ok && rp_file_contains("rp_agentcmp", "tool_events=51");
	ok = ok && rp_file_contains("rp_agentcmp", "scheduler_items=16");
	ok = ok && rp_file_contains("rp_agentcmp", "ranked_tasks=16");
	ok = ok && rp_file_contains("rp_agentcmp", "selected_tasks=7");
	...
	printf("rp_compare_plain: plain_kernel=passed objects=500 programs=24 state_files=71 acks=17 tools=53 status=ready\n");
	return 0;
}
