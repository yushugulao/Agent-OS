...
int main(void)
{
	...
	ok = ok && rp_file_contains("rp_modelver", "metric_artifact_count=52");
	ok = ok && rp_file_contains("rp_modeleval", "metric_evidence_coverage=1.000");
	ok = ok && rp_file_contains("rp_modeleval", "status=passed");
	ok = ok && rp_file_contains("rp_modeldep", "check_secret_policy=not_required");
	ok = ok && rp_file_contains("rp_modeldep", "status=ready");
	...
	ok = ok && rp_file_contains("rp_syssearch", "results=9");
	ok = ok && rp_file_contains("rp_sysscreen", "full_text_included=3");
	ok = ok && rp_file_contains("rp_sysextract", "risk_of_bias=3");
	ok = ok && rp_file_contains("rp_syssynth", "confidence=moderate");
	ok = ok && rp_file_contains("rp_sysprisma", "included=3");
	...
	ok = ok && rp_file_contains("rp_schedtask", "task=schedule-task:RUN-042:library-prep");
	ok = ok && rp_file_contains("rp_schedbook", "booking=schedule-booking:RUN-042:seq-library");
	ok = ok && rp_file_contains("rp_schedconf", "conflict=schedule-conflict:RUN-042:seq-01-overlap");
	ok = ok && rp_file_contains("rp_schedexec", "execution=schedule-exec:RUN-042:library-prep");
	...
	ok = ok && rp_file_contains("rp_traincomp", "open_gaps=0");
	...
	ok = ok && rp_file_contains("rp_review_dashboard", "backend_review_evidence=rp_backend_exec;plain_costs=7;agentos_replacements=7;risks=7;review_pack=rp_review_pack;status=ready");
	...
	ok = ok && rp_file_contains("rp_usableops", "handoff=usable-handoff:RUN-900:reviewer");
	ok = ok && rp_file_contains("rp_package", "usable_research=rp_usable;templates=3;datasets=3;library_sources=3;dag_stages=9;deliverables=8;status=ready");
	ok = ok && rp_file_contains("rp_web_bundle", "usable_research_page=rp_usable;templates=3;datasets=3;library_sources=3;dag_stages=9;queues=2;status=ready");
	ok = ok && rp_file_contains("rp_review_dashboard", "subsection=usable_research;source=rp_usable;checks=100;handoff=ready;status=ready");
	...
	ok = ok && rp_file_contains("rp_usableboot", "doctor=usable-platform-doctor:1;checks=10;passed=10;failed=0;warnings=0;status=ready");
	ok = ok && rp_file_contains("rp_usablescaf", "scaffold=scaffold:lab-gene-x:starter");
	ok = ok && rp_file_contains("rp_usablelaunch", "launch=usable-project-launch:lab-gene-x:1");
	ok = ok && rp_file_contains("rp_usablepack", "bundle=usable-project-bundle:lab-gene-x");
	ok = ok && rp_file_contains("rp_package", "usable_project=rp_usableproj;scaffolds=3;launches=2;bundles=2;doctor=pass;status=ready");
	ok = ok && rp_file_contains("rp_web_bundle", "usable_project_page=rp_usableproj;scaffolds=3;launches=2;bundles=2;doctor=pass;status=ready");
	ok = ok && rp_file_contains("rp_review_dashboard", "subsection=usable_project;source=rp_usableproj;checks=120;bundles=2;status=ready");
	...
	ok = ok && rp_file_contains("rp_prov_view", "agentos_mapping=kernel_timeline,kernel_provenance_edges,kernel_ledger,context_detail");
	...
	ok = ok && require_equal("backend_exec_passed", backend_exec_passed, 7);
	...
	ok = ok && rp_file_contains("rp_backend_exec", "case=agentos-ucore;source=rp_wfio;status=kernel_target");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_cases=7");
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_case=user-context;input=rp_query;artifact=rp_provpath;result=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_case=user-fsmeta;input=rp_artifact_manifest;artifact=rp_query;result=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_case=user-recovery;input=rp_retrylog;artifact=rp_fix;result=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_case=user-event;input=rp_worker+rp_timeline;artifact=rp_agent_run;result=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_case=user-audit;input=rp_audit+rp_provpath;artifact=rp_package;result=passed");
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_case=user-context;input=rp_query;artifact=rp_provpath;result=passed;reason=user_space_context_log;input_check=pass;artifact_check=pass");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_case=user-fsmeta;input=rp_artifact_manifest;artifact=rp_query;result=passed;reason=file_manifest_scan;input_check=pass;artifact_check=pass");
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_detail=user-context;src=rp_query+rp_provpath;req=context_path;obs=pass;act=rebuild_from_files;review=userland");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_detail=user-fsmeta;src=rp_artifact_manifest;req=metadata_index;obs=pass;act=scan_manifest;review=userland");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_detail=user-recovery;src=rp_retrylog+rp_fix;req=action_commit+artifact_update;obs=pass;act=repair_files;review=userland");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_detail=user-event;src=rp_worker+rp_timeline;req=event_wait_wake;obs=pass;act=file_event_records;review=userland");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_detail=user-audit;src=rp_audit+rp_provpath;req=audit+provenance;obs=pass;act=append_logs;review=userland");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_detail_rows=7");
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_report=user-context;plain_cost=rebuild_steps_6;agentos_replace=kernel_context_path;risk=untrusted_context;status=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_report=user-fsmeta;plain_cost=scan_records_128;agentos_replace=metadata_index;risk=scan_growth;status=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_report=user-recovery;plain_cost=manual_retry_contract;agentos_replace=capability_checked_action;risk=wrong_object_update;status=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_report=user-event;plain_cost=file_polling;agentos_replace=kernel_event_queue;risk=lost_handoff;status=passed");
	ok = ok && rp_file_contains("rp_backend_exec", "runner_report=user-audit;plain_cost=append_only_logs;agentos_replace=kernel_ledger_provenance;risk=tampered_context;status=passed");
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_observed=rp_stage_state,rp_retry_plan,rp_artifact_manifest,rp_llmeval,rp_query,rp_run_events,rp_audit");
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_passed=7");
	...
	ok = ok && rp_file_contains("rp_study", "study_metric=plain_ucore;file_scans=128;context_trusted=0;rebuild_steps=6;detail_checks=7;result=passed");
	ok = ok && rp_file_contains("rp_study", "study_metric=agentos_ucore;context_trusted=1;batch_tools=1;metadata_index=1;event_queue=1;recovery_tool=1;audit_ledger=1;detail_checks=kernel;result=kernel_target");
	...
	ok = ok && rp_file_contains("rp_report_text", "backend_evidence_report=rp_backend_exec;plain_costs=file_scan_manifest,retry_file_stage_file,rebuild_steps_6,scan_records_128,manual_retry_contract,file_polling,append_only_logs;agentos_replacements=batch_tool_context,event_context,kernel_context_path,metadata_index,capability_checked_action,kernel_event_queue,kernel_ledger_provenance;status=ready");
	...
	if (!rp_append_file("rp_agentcmp", "model_registry_service_checks=96;models=1;versions=1;evaluations=1;deployments=1;serving_checks=1;agentos_replacements=4;status=ready")) return 1;
	if (!rp_append_file("rp_agentcmp", "systematic_review_checks=104;protocols=1;searches=1;screening=9;extractions=3;bias=3;prisma=1;agentos_replacements=4;status=ready")) return 1;
	...
	if (!rp_append_file("rp_agentcmp", "statistical_design_checks=120;designs=1;power=underpowered;randomization=balanced;blinding=ok;stat_result=approved_with_sample_size_note;agentos_replacements=4;status=ready")) return 1;
	...
	if (!rp_append_file("rp_agentcmp", "portability_backend_checks=18;execution_plan=workflow-migration-execution-plan:RUN-042:agentcompare;backend_scenario=backend-scenario:RUN-042:agentcompare;compare_profile=compare-profile:RUN-042:migration;passed_cases=7;planned_cases=0;status=ready")) return 1;
	if (!rp_append_file("rp_agentcmp", "backend_runner_checks=21;runner_cases=7;runner_passed=7;runner_planned=0;plain_inputs=7;study_metrics=2;backend_runner_detail_checks=42;runner_detail_rows=7;backend_runner_report_checks=35;runner_report_rows=7;backend_report_links=2;status=ready")) return 1;
