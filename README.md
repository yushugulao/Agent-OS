...
The `host_tools/plain_ucore_reader.py` utility renders ordinary `rp_*` state files into host-viewable HTML pages and API JSON files. It consumes the `host_plain_ucore_v2` reader contract written by `rp_web_bundle`. The generated pages use a sidebar, page-level summary cards, research-output summaries, workflow execution view tables, workflow control view tables, workflow evidence link tables, Agent detail summaries, Agent roster and decision-flow tables, evidence-package summaries, evidence detail summaries, review dashboard sections and gates, claim/provenance/protocol tables, comparison summaries, compare metric summaries, operations report narrative tables, operations source-file tables, LLM Relay request/response/quality tables, plain-kernel signal tables, state tables, Run/Compare/Review Host action trace tables, action-output link and detail tables, action-impact tables, host-action history, and a batch-action editor for running the same research flow through plain uCore.
...
The Run, Compare, Review, Artifacts, and Actions pages also render action-output link, detail, and impact tables. These tables join `host-actions.jsonl` with the refreshed `rp_*` files, so a reader can see which Host action produced or changed `rp_input`, `rp_stage_state`, `rp_artifact_manifest`, `rp_llm_req`, `rp_review2`, `rp_package`, `rp_agentcmp`, and related files, including the matched state-file line that proves the visible result. The impact tables then point the action at report sections, artifact review paths, review gates, LLM packets, LLM quality rows, and package rows where those targets are available.

The artifact page now renders manifest records, the artifact dossier, derived sections, provenance records, artifact review path rows, dossier checks, archive files, stage logs, review/LLM signals, host artifact actions, and operations source-file links from ordinary `rp_*` files, so generated artifacts are visible as concrete input-output, report, review, and verification records instead of a single status line. `rp_artifact_manifest` writes `artifact_review_path` records for raw input to report, quality metrics to delivery package, and failed-stage recovery to reviewer package; `rp_consistency` also verifies the artifact path by checking the stage state, run event, retry plan, review gate, and LLM-quality source records.
...
The Run page also renders `Workflow Execution View`, `Workflow Control View`, and `Workflow Evidence Links` from existing workflow state files. These tables join `rp_stage_state`, `rp_cache_index`, `rp_retry_plan`, `rp_run_events`, `rp_worker`, `rp_execobs`, `rp_artifact`, `rp_artifact_manifest`, `rp_stage_log`, `rp_report_text`, `rp_review_dashboard`, and `rp_package` into readable stage, worker, cache, retry, observer, artifact, report, review, and delivery rows, so the failed align-stage retry and the host-triggered workflow run can be reviewed without manually opening every state file.
