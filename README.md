...
`rp_seed_orch` is the Host-action run entry. It runs the same forty-two platform programs, but first filters the captured Host action payload into compact per-program argv chunks. This keeps the unchanged uCore kernel and file-system builder intact while still letting the native user programs react to a real Host batch request.
