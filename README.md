...
- 汇总材料：默认放在 `results/latest/`，包括 `demo-guide.html`、`demo-checklist.html`、`experiment-design.html`、`runner-statistics.html`、`evidence-map.html`、`index.html`、`monitor.html`、`summary.csv`、`runner-sweep.csv`、`runner-statistics.csv`、`load-profile.csv`、`evidence-manifest.csv`、`demo-checklist.csv`、`experiment-design.csv`、`report.md` 和 `charts/*.svg`。该目录被 `.gitignore` 忽略，适合保留本机最新运行结果。
