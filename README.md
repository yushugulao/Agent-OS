...
The standalone `rp_test_suite` program writes `rp_tests` when it is run directly. The main orchestrated path keeps the current 711-check count and comparison result in `rp_agentcmp` so the full seeded run stays inside the upstream teaching file-system inode limit.
...
The artifact page now renders manifest records, the artifact dossier, derived sections, archive files, stage logs, and host artifact actions from ordinary `rp_*` files, so generated artifacts are visible as concrete input-output records instead of a single status line.
