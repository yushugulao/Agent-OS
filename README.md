...
`contest-demo` 只接受干净提交，不读取历史 `results/`，也不连接云 API。它构建一份固定
AgentOS 内核，并按预注册样本编号和执行顺序生成至少 8 份换序 Guest 工作负载镜像；每轮
都启动真实 RISC-V QEMU，同一 workflow 内交替运行 Compat 与 Native 路径。Guest 通过
ABI v2 输出静稳栅栏前后的原始计数、实际 workload syscall、时间戳和结果 hash，Host 再
校验原始 pair、observer 身份、源码与产物收据，计算 AB/BA 配对差值、比率和 95% 区间。
Dashboard 只展示这些实测值和逐字段计数口径，不用功能“通过”数量冒充性能证据。
...
`local-e3` 会校验精确的硬件、MSYS2 runtime、工具文件和配置身份。当前配置为
`provisional_requires_full_suite`，必须在最终冻结提交完成三轮重校准后才能重新启用本地时长门。受管输入、profile 或证据任一漂移
都会在进入 QEMU 前 fail closed。
...
| `agentfs_ucore` | 真实 inode 绑定、partial update、selector 一致性、`.agentmeta`、扫描/索引查询一致性、无内核查询结果缓存、内容摘要、有界去重预取、字段驱动批量状态维护、metadata 工作预算与交接端点生命周期。 | `metadata_action attempts=<次数> preemptions=<让出次数> graph_edges=<边数> generation_delta=<增量>`、`prefetch_hints=1 bounded=1 ... preemptions=<次数>`、`handoff_target_exit=1 endpoint_reuse=1 preemptions=<次数> ... clean=1`、`agentfs_ucore: parent passed` |
...
| `iobudget_ucore` | PUBLIC 速率/缓存压力、稳定 owner 归因、reserved/shared/device lease 上界、请求内物理传输批量结算、线程退出 lease 回收、scheduler 内核态中断交付、fault/异常退出清理归因与 debt 结算、workflow cache floor、CONTROL 保留预算和跨域有界进展。 | ABI v6；`work_conserving=shared-loan shared=<实际传输数> refills=<次数> throttles=0`、`iobudget_ucore: parent passed` |
