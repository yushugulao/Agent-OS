...
user/src/rp_revdash.c
user/src/rp_publication.c
user/src/rp_runbooks.c
user/src/rp_projectrel.c
user/src/rp_studyproto.c
user/src/rp_stdesign.c
user/src/rp_modelreg.c
user/src/rp_sysreview.c
user/src/rp_expsched.c
user/src/rp_traincomp.c
...
`rp_orch` runs 57 platform programs as separate uCore user processes:
...
- package,
- AiiDA-style calculation job records with computer, code, scheduler, retrieved files, parser result, export, and package linkage,
...
The comparison step also checks that workflow portability and backend execution name the same execution plan, backend scenario, and compare profile. It verifies seven passed plain-uCore cases, records the matching AgentOS replacement targets, and writes the study records consumed by the Host Reader. This publishes `portability_backend_checks=18` in `rp_agentcmp`.

The backend scenario now also writes compact case-runner evidence. In the plain uCore target it records seven executable user-space cases: plain-uCore execution-plan checking, ordinary retry recovery, user-space context reconstruction, manifest-based metadata lookup, user-space recovery records, file-backed event observation, and append-only audit/provenance files. The enhanced AgentOS-uCore target runs the same RUN-042 workflow under role-specific Agent children. Main stages append ten facts to `rp_agentos_mainflow`: trusted Context, indexed file metadata query, Agent event notification, `rerun_stage` and `write_report`, kernel ledger/provenance reads, sentinel permission denial, timeline observation, workbench file verification through kernel metadata, package provenance through the kernel ledger, and real-task report/audit records through kernel Context. Backend, consistency, metrics, compare, and test-suite programs consume that file, so the enhanced target does not only record that Agent tests ran; the same research flow depends on kernel services for querying, recovery, coordination, access control, workbench verification, package tracing, real-task answer audit, and audit evidence. The backend links this evidence into `rp_runner`, `rp_report_text`, and the review handoff state, then publishes `backend_runner_checks=21`, `backend_runner_detail_checks=42`, `runner_detail_rows=7`, `backend_runner_report_checks=35`, `runner_report_rows=7`, and `backend_report_links=2` in `rp_agentcmp`. The Run page derives case narratives from `rp_backend_exec`, so a reviewer can see the plain-uCore cost and AgentOS replacement without switching to the comparison page.
