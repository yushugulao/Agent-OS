...
The Run page also renders `Workflow Execution View` from existing workflow state files. That table joins `rp_stage_state`, `rp_cache_index`, `rp_retry_plan`, `rp_run_events`, `rp_worker`, and `rp_execobs` into one view, so the failed align-stage retry and the host-triggered workflow run can be reviewed without manually opening every state file.
