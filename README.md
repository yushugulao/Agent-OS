...
The Run, Compare, Review, Artifacts, and Actions pages also render action-output link and detail tables. These tables join `host-actions.jsonl` with the refreshed `rp_*` files, so a reader can see which Host action produced or changed `rp_input`, `rp_stage_state`, `rp_artifact_manifest`, `rp_llm_req`, `rp_review2`, `rp_package`, `rp_agentcmp`, and related files, including the matched state-file line that proves the visible result.
