...
本文是 [design.md](design.md) 的任务四细节附录，重点说明 uCore 分支当前实现的文件元数据表、真实 inode 关联、私有 `.agentmeta` 元数据文件、属性查询、索引路径、内容摘要工具、依赖查询和查询历史驱动的预取提示。
...
`agentfs_ucore` 会创建额外真实文件，绑定自定义元数据，并验证重新调用 `agent_file_meta_init()` 时自定义数据来自 `.agentmeta` 重新加载，而不是被默认表覆盖。它还会验证字段清空、文件删除清理、selector 未命中、scan/index 返回语义一致、结果超过 `max_hits` 时设置 `truncated`，并生成接近 128 条真实文件元数据，让扫描路径和索引路径的 `scanned_records` 差异明显。重复执行同一个非强制扫描查询时，它会验证 `CACHE_HIT` 原因位；随后更新文件状态，确认旧代数缓存不会继续返回过期结果。
...
| 字段 | 含义 |
| --- | --- |
| `plan` | 0 表示扫描，1/2/3 分别表示 status/stage/kind 索引 |
...
这个设计面向 Agent 的常见访问模式：同一个页面、同一个 Agent worker 或同一轮多 Agent 协作，经常会重复读取“某 run 的 failed artifact”“某 stage 的输出”“恢复后的 report”。内核用 generation-aware cache 避免重复走索引链，同时保留强制扫描路径用于证明结果一致。
...
metadata 查询说明“哪个文件符合条件”，内容摘要工具说明“这个真实文件的开头内容和内容指纹是什么”。`AGENT_TOOL_READ_FILE_DIGEST` 使用 `read_file_digest` 工具名，输入为 `selector:string`。selector 可以是物理文件名、逻辑路径、stage，也可以是 `project=...;run_id=...;stage=...;status=...` 形式的属性过滤串。属性过滤命中多条时读取第一条命中文件。
...
`read_file_digest` 还维护 8 槽内核 digest cache。缓存 key 是真实 inode 身份和文件元数据代数：
...
缓存 value 保存文件大小、参与计算字节数、FNV-1a 指纹和短预览。重复读取同一个绑定了 Agent metadata 的真实文件时，第二次可以直接复用缓存结果。文件写入、截断、删除或 metadata 更新会更新 `fs_generation`，旧缓存条目会被跳过。

未绑定 Agent metadata 的普通文件不会进入 digest cache。这样做是为了避免普通文件同尺寸改写时，Agent 子系统缺少可靠 generation 信号而返回过期内容证据。缓存计数通过 `agent_info.file_digest_cache_hits` 和 `agent_info.file_digest_cache_misses` 暴露，供测试和性能材料直接引用。

## 查询历史驱动的预取提示

文件查询命中后，内核会把本次查询视为 Agent 当前探索路径的一部分，并根据源文件的 stage 依赖推导后续可能需要关注的文件 metadata。例如查询 `RUN-042` 的 `align` 阶段后，如果依赖关系显示 align 会影响 analyze、report 和 archive，内核会在同一 project/workflow/run 中查找这些后续阶段的元数据，并生成预取提示。

预取提示的生成条件：

1. 当前进程必须是 Agent；
2. 文件查询必须至少命中一条记录；
3. 命中的 source 文件有 `dependency_mask`，或可由 stage 推导依赖关系；
4. target 文件与 source 位于同一 project/workflow/run；
5. target stage 位于 source 的依赖集合中；
6. target stage 可以通过 stage 索引解释候选记录数量。
...
`agent_file_meta_set()` 更新文件状态时，会检查是否有 Agent 注册了匹配 watch。如果某个 Agent 监听 `status=failed`，更新为 failed 的文件会产生 `AGENT_EVENT_FILE_STATUS`，并唤醒等待中的 Agent。事件会携带触发本次状态变化的 cause/span，目标 Agent 消费事件后可以把后续查询和恢复动作接到同一因果链。
...
- 可输出由历史查询和阶段依赖生成的 metadata 预取提示；
...
- investigator 能通过内核交接的预取提示读取摘要、真实日志 digest、依赖和后续阶段摘要；
- recovery 后能查询报告文件；
- 查询路径使用索引。
...
| 限制项 | 说明 |
| --- | --- |
| 元数据来源 | 当前来自私有 `.agentmeta`、默认真实文件、`agent_file_meta_set()` 和根目录自动扫描 |
