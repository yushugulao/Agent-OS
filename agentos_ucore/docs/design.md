...
| 5 | 可扩展性 | 文件元数据字段、`agentos:event`、Context Path、Loop 状态、工具表、cause/span 因果链、统一 timeline 和 Run Ledger 摘要可继续扩展到最终 LLM Gateway 和可视化大屏 |
...
| 当前限制 | 尚未实现多级目录递归扫描、云端 LLM Gateway、宿主机可视化大屏 |
...
| 文件查询引擎 | Agent 子系统维护 128 条文件元数据，主键使用 `dev + inum`，提供扫描路径、status/stage/kind 索引路径、查询计划解释、私有 `.agentmeta` 元数据文件、调度器空隙分批根目录扫描和同一 span 的预取提示查询 |
...
| Agent 核心逻辑 | `os/agent.c` | Agent 初始化、工具执行、Context Path、文件元数据、自动扫描、事件等待和调度评分 |
...
flowchart TB
    ...
    FS["nfs/fs.img"]
    Tests["agentfinal_ucore / agentfs_ucore / agentscan_ucore / agentloop_ucore / agentsched_ucore / agentbench_ucore / labdemo_ucore / agentsecurity_ucore"]
...
这个设计让演示中的 “sentinel 发现失败 -> investigator 查询原因 -> recovery 恢复” 不只是几段串口输出，而是能在内核 Context 与事件结构里保留可追踪的前后关系。完整性链进一步证明相邻记录的顺序由内核维护：第一条记录 `prev_hash=0`，后续记录的 `prev_hash` 必须等于上一条可见记录的 `record_hash`，header 中的 `latest_record_hash` 等于最新记录 hash。跨 Agent 事件中的 cause sequence 需要结合 source pid 与 span 解释；它不是全局唯一整数，也不是磁盘持久化审计日志。
...
| Context Path 容量 | 固定 128 条环形记录，每条包含 16 字节 payload/result 短文本摘要和 prev/record hash，并在内核 PCB 中保存最近 128 条完整请求/响应详情 | 可证明 FIFO 淘汰、相邻记录顺序和可审计详情；Context 尾部留给用户自管 cache | 更长历史需要后续持久化 |
...
| Run Ledger 摘要 | `agent_ledger_snapshot()` 返回全局审计链尾 hash、sequence 范围和分类计数 | 让最终页面用一个小结构确认当前全局短记录仍属于同一条内核维护的运行事实链 | 当前不是跨重启持久化证明，也不带签名 |
...
| 文件内容摘要 | `read_file_digest` 受 `CONTENT_READ` capability 控制，按 selector 读取真实文件短预览、最多 4096 字节内容和 FNV-1a 指纹；绑定 Agent metadata 的真实文件进入 8 槽 generation-aware digest cache | 让 Agent 在 metadata 命中后取得轻量内容证据，重复读取同一文件证据时复用结果，并自动进入 Context/timeline | 不是全文搜索，不建立内容倒排索引；未绑定 Agent metadata 的普通文件不缓存 |
| 文件预取提示 | 文件查询命中后按阶段依赖生成每 Agent 8 条 metadata 提示，并写入同一 span 的 32 条全局提示总线；message 入队时可由内核交接给接收者 | 把 Agent 历史查询路径转化为内核可见的下一步候选，并让同一因果链上的 Agent 直接查询跨 Agent 提示，贴合赛题“预测性预取”方向 | 当前只提示 metadata，提示本身不预读文件内容 |
