...
int main(void)
{
	...
	ok = ok && rp_file_contains("rp_backend_exec", "passed_cases=4");
	...
	ok = ok && rp_file_contains("rp_ui_compare", "agentos_kernel=observed");
	...
	ok = ok && rp_file_contains("rp_api_compare", "agentos_kernel=observed");
	...
	ok = ok && rp_file_contains("rp_web_bundle", "reader_views=25");
	...
	ok = ok && require_equal("backend_exec_passed", backend_exec_passed, 4);
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_observed=rp_stage_state,rp_retry_plan,rp_artifact_manifest,rp_llmeval,rp_agentos_kernel");
	...
	ok = ok && rp_file_contains("rp_backend_exec", "runner_passed=4");
	...
	ok = ok && rp_file_contains("rp_study", "study_metric=agentos_ucore;context_trusted=1;batch_tools=1;metadata_index=1;detail_checks=kernel;result=passed");
	...
	ok = ok && rp_file_contains("rp_study", "agentos_kernel=observed");
	...
	if (ack_count < 55 || tool_count < 210) {
		printf("rp_compare_plain: bad_event_counts acks=%d tools=%d\n", ack_count, tool_count);
		return 1;
	}
	if (!rp_append_file("rp_agentcmp", "plain_kernel=passed;programs=55;state_files=191;message_acks=55;tool_events=210;action_state_records=12;test_cases=1432;action_side_effect_records=16;service_page=1;llm_queue_checks=3;llm_guard_checks=3;review_dashboard=1;review_pack=1;runbook_service_checks=16;project_delivery_checks=18;study_protocol_checks=20;operations_board_checks=18;review_board_checks=24;control_plane_checks=30;integrity_plane_checks=36;coherence_plane_checks=40;publication_checks=48;mature_capability_checks=72;provenance_view_checks=64;workbench_exports=7;dynamic_inputs=4;host_ui_events=10;reader_contract=1;advanced_surface_objects=5;startup_health_checks=8;startup_doctor_checks=14;research_product_checks=18;runtime_assurance_checks=24;research_ops_checks=28;regulated_research_checks=32;lab_governance_ops_checks=26;state_catalog_checks=12;knowledge_index_checks=22;llm_transcript_checks=3;workbench_delivery_checks=15;research_portfolio_checks=16;execution_scale_checks=14;operations_scale_checks=12;project_revision_incident_checks=12;reserved_research_surface_checks=21;root_state_surface_checks=10;agentos_reserved_surface_checks=21;status=ready")) return 1;
