...
def plot_cold_warm_grouped(root: Path, plt: Any, np: Any, pd: Any) -> tuple[Any, dict[str, Any]]:
    diagnostics = _read_table(root, "agenteval_diagnostics", pd)
    samples = _read_table(root, "agenteval_samples", pd)
    _require(diagnostics, ["experiment", "load", "cache", "duration_us"], "cold/warm grouped")
    _require(samples, ["experiment", "load", "variant", "operations", "duration_us"], "cold/warm grouped")
    diagnostics = diagnostics[
        (diagnostics["experiment"] == "file_query_table_ablation")
        & (diagnostics["cache"] == "cold-rebuild")
    ].copy()
    samples = samples[
        (samples["experiment"] == "file_query_table_ablation")
        & samples["variant"].isin(["scan", "index"])
    ].copy()
    diagnostics = _numeric(diagnostics, ["load", "duration_us"], pd)
    samples = _numeric(samples, ["load", "operations", "duration_us"], pd)
    # Match the cold diagnostic's single query rather than mixing operation counts.
    samples = samples[samples["operations"] == 1]
    diagnostics["category"] = "Cold index rebuild"
    diagnostics["us_per_query"] = diagnostics["duration_us"]
    samples["category"] = samples["variant"].map({"scan": "Warm scan", "index": "Ready index"})
    samples["us_per_query"] = samples["duration_us"] / samples["operations"]
    data = pd.concat(
        [diagnostics[["load", "category", "us_per_query"]], samples[["load", "category", "us_per_query"]]],
        ignore_index=True,
    ).dropna()
    categories = ["Cold index rebuild", "Warm scan", "Ready index"]
    ...
    ax.set_title("Cold rebuild vs warm scan vs ready index")
    ax.grid(axis="y")
    ax.spines[["top", "right"]].set_visible(False)
    ax.legend(ncols=3, loc="upper left")
    _footnote(fig, "Cold includes index reconstruction; warm lanes use one hit-producing query; bars are medians, whiskers IQR.")
    return fig, {"rows": int(len(data)), "loads": [int(value) for value in loads], "operation_count": 1}
...
def plot_task_distribution(root: Path, plt: Any, np: Any, pd: Any) -> tuple[Any, dict[str, Any]]:
    ...
    for ax, frame, metric, title, ylabel in (
        (axes[0], sequences, "duration_us", "Sequence latency", "Sequence duration (us)"),
        (axes[1], operations, "service_start_interval_tick", "Service-start intervals", "Interval (scheduler ticks)"),
    ):
        ...
        ax.grid(axis="y")
        ax.spines[["top", "right"]].set_visible(False)
    fig.suptitle("Batch, scalar, and Task SQ-CQ latency distributions", fontsize=12, fontweight="bold")
...
def plot_jain_fairness(root: Path, plt: Any, np: Any, pd: Any) -> tuple[Any, dict[str, Any]]:
    jain = _read_table(root, "eevdf_jain", pd)
    _require(jain, ["scenario", "n", "sum", "sum_sq"], "Jain fairness")
    jain = _numeric(jain, ["scenario", "n", "sum", "sum_sq"], pd).dropna(
        subset=["scenario", "n", "sum", "sum_sq"]
    )
    jain = jain[jain["scenario"].isin([1, 2, 3, 4])].copy()
    jain["fairness"] = (jain["sum"] ** 2) / (jain["n"] * jain["sum_sq"])
    jain = jain[np.isfinite(jain["fairness"])]
    if set(jain["scenario"].astype(int)) != {1, 2, 3, 4}:
    ...
    for scenario, lane in jain.groupby("scenario"):
        ax.scatter(np.full(len(lane), scenario), lane["fairness"], s=26, color="#2C7FB8", alpha=0.38)
    ...
    lower = min(0.9, max(0.0, float(jain["fairness"].min()) - 0.02))
    ...
    _footnote(fig, "J=(sum x)^2/(n sum x^2), with x=service_cycles/1024 from the emitted raw integer inputs.")
    return fig, {"concurrency_levels": [1, 2, 3, 4], "rows": int(len(jain)), "basis": "service_cycles_div_1024"}
...
def _heatmap_panel(ax: Any, pivot: Any, title: str, cmap: str, np: Any) -> None:
    ...
    ax.set_xticks(range(len(pivot.columns)), [str(value).replace("_", "\n") for value in pivot.columns], rotation=0)
...
def plot_io_heatmap(root: Path, plt: Any, np: Any, pd: Any) -> tuple[Any, dict[str, Any]]:
    ...
    _require(contest, ["path", "metric", "per_workload_syscall"], "I/O heatmap")
    ...
    image = _heatmap_panel(axes[0], contest_pivot, "Kernel and VFS counters per workload syscall", "YlGnBu", np)
    _heatmap_panel(axes[1], task_pivot, "Task ABI work per completed operation", "YlGnBu", np)
    fig.suptitle("Kernel I/O and workload-normalized intensity", fontsize=12, fontweight="bold")
    colorbar = fig.colorbar(image, ax=axes, shrink=0.65, pad=0.02)
    colorbar.set_label("Within-metric relative intensity")
    _footnote(fig, "Cell text is the median raw normalized value; color is scaled within each metric, so unlike units are not compared.")
    return fig, {"contest_metrics": list(contest_pivot.columns), "task_metrics": list(task_pivot.columns)}
