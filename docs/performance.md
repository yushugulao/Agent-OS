...
16 组配对中，索引查询的核心阶段都更短，配对差值中位数为 `-23.4415 毫秒`，中位加速比为 3.118 倍；索引把每次需要检查的记录从 97 条缩减到 2 条，因此核心阶段稳定受益。把准备与收尾步骤计入后，完整流程的配对差值变为 `+13.452 毫秒`，约慢 `1.90%`，索引查询只有 3/16 组配对更快。逐组扣除核心阶段后，`indexed` 的核心外余量 16/16 都更长，中位多 `33.477 毫秒`，范围为 `+5.975` 至 `+49.563 毫秒`。这些结果表明核心阶段以外的总开销抵消了索引收益。后续分段计时将单独识别场景启动、文件与元数据准备、统计输出和清理的开销，便于针对主导项优化。

[![同次启动下两种查询方式的用时对比](figures/performance/01_paired_core_performance.png)](figures/performance/01_paired_core_performance.pdf)

16 组配对结果全部由索引查询胜出，整体核心阶段中位加速比为 3.118 倍；索引缩小候选集合后，查询、恢复写入和结果复核组成的核心路径随之缩短。`AB` 与 `BA` 两组的中位加速比分别为 1.454 倍和 5.481 倍，索引核心用时的四分位区间为 `6.5185` 至 `28.9128 毫秒`，表明收益方向稳定，具体倍数和离散程度会受运行顺序及前序状态影响。性能回归继续在同次启动内配对并轮换顺序，以核心路径是否持续缩短评估索引效果。
...
12 组实测中位加速比介于 1.164 倍和 2.808 倍之间，目录规模从 24 条增至 96 条时，索引的总体收益随候选集合扩大而上升；遍历查询需要检查更多目录记录，索引查询则把工作集中到命中集合。命中数与加速比并不单调，例如目录为 64 条时，命中 1、2、4、8 条的结果依次为 2.032、1.995、2.534、2.361 倍；按逐对样本统计，24、64、96 条目录中 `indexed` 较快的数量分别为 43/60、60/60 和 58/60。计时区间从索引就绪后的查询开始；查询规划器应结合目录规模、预期命中数和索引生命周期选择路径。
...
504 条唤醒采样中有 425 条为 0 tick、79 条为 1 tick，各并发组的 Jain 公平指数中位数均不低于 `0.99998`；`EEVDF` 在工作流可运行后依据 `eligible` 状态和 `virtual deadline` 分配外层 CPU 服务量，使 1 至 4 个并发工作流的 `service_cycles` 接近均分。一个工作流放大到 4 个忙线程时，Jain 中位数为 `0.9980725`，该工作流获得 `26.895%` 服务量，高于四个工作流均分时的 `25%`，说明工作流级记账大体抑制了线程放大，但并非绝对无偏。数据清单中 boot 05 的 3 条 sample marker 因串口拼接缺少 wake-bucket 尾字段，其 `service` 值依然完整；剔除受影响的测试启动数据后，并发度 3 和 4 的 Jain 中位数分别为 `0.9999911` 和 `0.9999842`，定性结论不变。精确唤醒分布来自独立的逐探针表。测试条件为单 Hart、并发上限 4，16 个工作流分批到达；后续测试将增加更细粒度时钟、多 Hart 负载和线程数不对称场景。
