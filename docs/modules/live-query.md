...
一个工作流会不断产生检索结果、检查点和报告。只看文件名，无法得知文件处于哪个阶段、是否可用、依赖哪些数据，也无法分辨 inode 删除后又被新文件复用的情况。为此，AgentOS 在 uCore 内核中维护 Metadata Catalog（元数据目录），并把索引查询、文件身份、Typed Watch（类型化订阅）和编辑租约放在一起管理。
