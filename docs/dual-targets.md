...
科研流程的项目名、run id、阶段名称、失败原因和恢复策略由用户态程序写入。结构检查会扫描根目录 `os/`，防止这些演示常量变成内核默认业务。
...
也可以进入 `baseline_ucore/` 后运行普通目标：

```bash
make user nfs/fs.img TOOLPREFIX=riscv64-linux-gnu- CHAPTER=platform
make run TOOLPREFIX=riscv64-linux-gnu- LOG=error INIT_PROC=rp_orch CHAPTER=platform
```
...
根目录是 AgentOS-uCore 主作品。增强内核能力应放在根目录 `os/`、`user/` 和 `scripts/` 中，便于评审者进入仓库后直接查看主要实现。
