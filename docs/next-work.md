...
- Test growth: keep increasing small tests or check programs around core services, exported UI data, Host Web/API payload files, file-backed action records, demo run artifacts, custom research input fields, request-form sections, uploaded-material sections, reusable-source sections, bibliography records, citation-plan records, delivery manifests, review pages, data pipeline files, package export indexes, Host UI render data, service surface records, workflow runner execution files, workflow runner detail fields, Agent collaboration records, LLM relay records, and AgentCompare records. The current uCore branch already runs a 241-check user-space suite.
