...
The current branch has a working native baseline. It now includes active cross-file consistency checks, concrete RUN-042 artifact files, data ingestion records, dataset snapshots, data previews, quality results, transform records, dataset collection export, workflow runner execution files, Agent collaboration records, file-backed Host LLM Relay protocol files, host UI export files, Host Web/API payload files, and an 82-check user-space test suite over ordinary uCore state files. The remaining work is to keep moving behavior from compact records into active user-space services without changing the kernel.
...
- Web UI data: uCore does not need to serve HTTP directly, but it should produce ordinary files for a host UI home page, run detail view, Agent detail view, evidence detail view, data view, artifact view, comparison metrics view, route manifest, and API payloads.
...
- Test growth: keep increasing small tests or check programs around core services, exported UI data, Host Web/API payload files, demo run artifacts, data pipeline files, workflow runner execution files, Agent collaboration records, LLM relay records, and AgentCompare records. The current uCore branch already runs an 82-check user-space suite.
...
7. Continue expanding uCore user-space exports for UI pages and run details so the host web service can render the same scenario from plain-kernel output files. The current branch already exports home, run, Agent, evidence, data, comparison page data, route manifest, and API payload files.
