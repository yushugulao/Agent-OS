...
- Test growth: keep increasing small tests or check programs around core services, exported UI data, Host Web/API payload files, file-backed action records, demo run artifacts, custom research input fields, data pipeline files, package export indexes, service surface records, workflow runner execution files, Agent collaboration records, LLM relay records, and AgentCompare records. The current uCore branch already runs a 154-check user-space suite.
