...
The current branch has a working native baseline. It now includes active cross-file consistency checks, concrete RUN-042 artifact files, three custom research tasks stored in ordinary files, nine small CSV-style rows, request-form and uploaded-material sections inside `rp_input`, three derived custom run summaries, data ingestion records, dataset snapshots, data previews, quality results, transform records, dataset collection export, workflow runner execution files with command/output records, dependency checks, retry reasons, cache policy, support links, Agent collaboration records, file-backed Host LLM Relay protocol files, bio service files, lab resource service files, publication service files, knowledge service files, runtime service files, package export indexes for report, evidence, provenance, reviewer delivery, raw artifacts, and review page sections inside `rp_package`, host UI export files with navigation, timeline, artifact preview, Agent decision, evidence preview, and comparison metric rows, Host Web/API payload files, file-backed POST action request and response records, and a 221-check user-space test suite over ordinary uCore state files. The remaining work is to keep moving behavior from compact records into active user-space services without changing the kernel.
...
- Test growth: keep increasing small tests or check programs around core services, exported UI data, Host Web/API payload files, file-backed action records, demo run artifacts, custom research input fields, request-form sections, uploaded-material sections, delivery manifests, review pages, data pipeline files, package export indexes, Host UI render data, service surface records, workflow runner execution files, workflow runner detail fields, Agent collaboration records, LLM relay records, and AgentCompare records. The current uCore branch already runs a 221-check user-space suite.
