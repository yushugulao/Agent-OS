...
The current branch has a working native baseline. It now includes active cross-file consistency checks, concrete RUN-042 artifact files, data ingestion records, dataset snapshots, data previews, quality results, transform records, dataset collection export, workflow runner execution files, Agent collaboration records, file-backed Host LLM Relay protocol files, bio service files, lab resource service files, publication service files, knowledge service files, runtime service files, host UI export files, Host Web/API payload files, file-backed POST action request and response records, and a 122-check user-space test suite over ordinary uCore state files. The remaining work is to keep moving behavior from compact records into active user-space services without changing the kernel.
...
- Test growth: keep increasing small tests or check programs around core services, exported UI data, Host Web/API payload files, file-backed action records, demo run artifacts, data pipeline files, service surface records, workflow runner execution files, Agent collaboration records, LLM relay records, and AgentCompare records. The current uCore branch already runs a 122-check user-space suite.
