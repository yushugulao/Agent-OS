...
The current branch has a working native baseline. It now includes active cross-file consistency checks over ordinary uCore state files. The remaining work is to keep moving behavior from compact records into active user-space services without changing the kernel.
