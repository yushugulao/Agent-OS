...
The current branch has a working native baseline for the research Agent platform on an unchanged uCore kernel. It runs 66 platform programs, writes 245 compact state files, emits 300 tool-event records, publishes 36 Host Reader views, and exposes a 2500-check user-space test suite through `rp_tests` and `rp_agentcmp`.
