...
模型选择工具后，Harness 先封存输入 Artifact，并用 128 字节 descriptor 绑定 lifecycle、发起 Agent、parent Task、correlation、实际 tool id、所需 capability、允许工具、workspace revision、预算、deadline 与结果类型。长期 Guest 验证这些字段并向原生 SQ 提交；普通 Provider Agent 通过阻塞 claim 取得 Task，Host 受控 Provider 执行文件、构建或测试操作，再由 Provider Agent 封存结果 Artifact 并 complete。owner 从 terminal CQE 取得 handle，复核 producer、Task、lifecycle、长度与 SHA-256 后追加 TOOL Context，模型下一轮只消费已经结算的投影。Host 不直接把 Python 返回值写成工具终态。
