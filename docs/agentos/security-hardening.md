...
某次发布的 18-case 校准状态、动态通过项和最终指标只由 `evidence/releases/INDEX.md` 选中的 release bundle 判定；bundle manifest 必须绑定被测代码提交 C、命令、原始/规范化日志和校验结果。INDEX 未指向完整且验证通过的 bundle 时，本文列出的 marker 只能解释验收合同或历史记录，不能作为当前 HEAD 已通过的证据；远程 Runner 是否执行同理由 bundle 的 remote attestation 字段决定。
