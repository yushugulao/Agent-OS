...
| `831823e` | 块设备队列和全局 buffer cache 没有持久主体/workflow 公平边界；内核态 yield loop 可长期屏蔽 timer/device 中断，fault 退出还可在 `freethread()` 前后丢失清理 I/O 账本 | 稳定 owner/class 的 lease/token/debt 与排队 shared grant 轮转；ABI v5 以每次真实 `disk_submit` 为唯一物理计费边界，shared 与设备根同时扣减而不叠加容量；准入 account endpoint 不带债，reserved 设备端只由真实 account lease 背书带 debt；已接纳的有界原子多传输可形成有上界的 owner lane debt并由 checkpoint/teardown settlement 清偿；global device debt 对非保护 lane 是 gate，保护 lane可跨 lifecycle，并由设备根 refill 清偿且受 request/aggregate envelope 限制；每轮 scheduler idle kerneltrap 中断窗口、terminal cleanup I/O/kernel-work 上下文、按 sponsor 设置的 cache floor/cap 与 exclusive holder，以及 FS atomic/quiescent checkpoint、grouped qmap claim、FIFO metadata submit lane 共同约束跨预算生命周期 | 旧 ABI v3 的 marker 和单例耗时只属于历史轮，不授予当前候选通过状态；ABI v5 的 `iobudget_ucore`、VirtIO fault 与 allocator fault 结果必须从同一冻结 C 的原始日志和 release bundle 读取 |
...
18-case 时长门的校准状态由版本化 `ci/kernel-budgets.json` 判定。`04c1e6652324` 的三轮结果和 `evidence/calibrations/04c1e6652324/` 只属于该历史提交；当前配置状态为 `calibrated_full_suite`，绑定 `a9e7c67feda5` 的三轮完整样本、57 份执行 attestation 和源码指纹。受管输入或 profile 变化时不得携带该 baseline、limit、samples 或 fingerprint。校准包严格是未签名本地 E3 校准证据，不等于当前发布验收、GitLab CI、远程 Runner 或 E4。某次发布的完整动态通过项和最终指标只由 `evidence/releases/INDEX.md` 选中的 release bundle 判定；bundle manifest 必须绑定被测代码提交 C、命令、原始/规范化日志和校验结果。INDEX 未指向完整且验证通过的 bundle 时，本文列出的其他 marker 只能解释验收合同或校准记录，不能作为当前 HEAD 已完成完整 E3 发布验收的证据；远程 Runner 是否执行同理由 bundle 的 remote attestation 字段决定。
