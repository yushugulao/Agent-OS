...
用户代码保持 RX，数据和栈保持 RW+NX。Context 前 6 页由内核写入、用户只读，
第 7 页是用户可写 cache，全部不可执行；完整请求、响应和来源归属位于按活跃
Agent 分配的私有 sidecar。普通进程和未使用 Agent 功能的进程不承担这部分内存。

内核将可信映射、用户 cache 和 sidecar 作为一个资源申请统一接纳和结算。
单拷贝只读视图删除了 shadow 同步成本，用户 cache 不能改变内核身份、历史或授权结果。
