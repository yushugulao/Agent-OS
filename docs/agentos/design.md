...
| 文件对象查询引擎 | 物理 metadata 表固定 512 条：SYSTEM 64 条，最多 4 个 ACTIVE/CLOSING/RETIRING workflow 各 112 条；同一 resolver 在 scope 内聚合 fid、物理/逻辑路径和完整 `dev + inum + incarnation`，分裂命中不得选边；依赖记录每 scope 16 条；每次查询真实执行扫描或索引候选遍历，本次调用的精确 hit slot 供预取核验 selector、一次扫描文件表并最多发布 8 个唯一目标 |
...
Metadata catalog 的 512 个物理槽采用冻结期固定分区：SYSTEM 独占 64 个，普通区 448 个由最多 4 个 workflow 各自独占 112 个。ACTIVE、CLOSING 和 RETIRING 共同计入这 4 个准入槽；RETIRING 在目录回收完成前继续占住原分区，新 workflow 不得复用它的份额。workflow 的文件系统 inode 账户同样以 112 为上限，因此目录记录与可发布 inode 具有相同的静态边界。该设计不计算跨 scope 的全局 union/max，不新增 catalog resource kind、backing lease 或 metadata envelope 账本，避免恢复、回收与创建路径维护第二套所有权状态。权威 alloc/edit/restore 均在 catalog 内重验 SYSTEM 64、scope 112、ordinary 448、唯一键和 lifecycle；edit 不能改换 scope，删除必须走统一 clear 路径。容量耗尽返回 `NO_SPACE`，lifecycle 改变返回可重试状态，键冲突和持久化不确定分别保留 `CONFLICT` 与 `INDETERMINATE`，不能塌缩成通用 I/O 错误。action status batch 也独立限制为 112 条，选择溢出会在 mutation 前返回 `NO_SPACE`。
