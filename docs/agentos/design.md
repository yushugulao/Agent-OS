...
| CI 时间预算 | 只统计完整 18 个 QEMU case 的 monotonic 运行时，不含编译 | `814021ab9dac` 曾在固定 runner 串行取得三轮 18-case，历史中位基线 `310.491647311s`、上限 `327.10s`。当前 candidate 的相关输入已经变化，配置为 `provisional_requires_full_suite`，旧 baseline、limit、samples 和 fingerprint 不得复用；最终 E3/E4 状态只由重新生成的 release bundle 与远端 attestation 判定。 |
