...
| CI 时间预算 | 只统计完整 18 个 QEMU case 的 monotonic 运行时，不含编译 | `04c1e6652324` 的三轮样本仅是该历史提交的未签名本地 E3 校准。当前配置为 `provisional_requires_full_suite`；冻结并重新校准后才能恢复 baseline/limit。最终发布 E3/E4 状态仍只由 C→E release bundle 与远端 attestation 判定。 |
