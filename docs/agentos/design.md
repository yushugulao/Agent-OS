...
| CI 时间预算 | 只统计完整 18 个 QEMU case 的 monotonic 运行时，不含编译 | 当前 `calibrated_full_suite` 配置绑定 `a9e7c67feda5` 的三轮样本、57 份执行 attestation 和受管源码指纹。最终发布 E3/E4 状态仍只由 C→E release bundle 与远端 attestation 判定。 |
