...
1. 冻结代码和验收语义，清理工作区，提交代码 C。C 必须包含 runner、测试、预算和文档，
   但不包含声称来自 C 的尚未生成发布包。
2. 从干净 C 运行 `python3 -I -S scripts/trusted-python-entry.py scripts/capture-final-evidence.py collect`。采集器在 detached C 中执行
   `make full-verify`，失败时不得留下 ready release。
3. 用 `python3 -I -S scripts/trusted-python-entry.py scripts/capture-final-evidence.py verify` 校验引用和 SHA256。人工核对所有 E2 marker、失败
