...
| H-02 | P0：部分综合验收程序硬编码“通过证据” | 普通目标的 backend 只输出 `demo_reference`，显式记录 `runtime_cases=0`、`performance_samples=0`；coherence/decision-support 双目标产品及聚合行统一为 `demo_reference/demo_expected/reference_ready`。AgentOS backend 的 receipt 从实际 source read 和 kernel check 累积，并在输出前冻结。Comparator 和 Reader 分离非证据兼容、参考目录、来源绑定 runtime；run-dual 拒绝外部 Guest 日志/命令/receipt 注入并自行运行 targeted case。`exact-field-v1` 以 128 B 分块流式读取源文件，允许长无关字段和跨块长 key，只接受精确且唯一的 `key=value`；空 key/value、CR、NUL、重复目标和前后缀伪匹配 fail closed。Reader action runner明确区分 clean/build/guest：前两阶段只依据退出码，QEMU 启动后才按规范化完整日志行识别 Guest 故障。 | `test_backend_evidence_contract.py` 与 `test_reference_catalog_contract.py` 拒绝伪 pass/ticks、残缺 reference envelope、常量 receipt、绕过读取和提前成功；`test_dual_measurement_source_contract.py` 拒绝重新引入外部日志复用；`test-rp-evidence-file-field.py` 的 ASan/UBSan Host probe 覆盖长字段、长 key、重复、CR/NUL、非精确匹配和空输入，并纳入 `ci-check`；`test_plain_ucore_action_runner.py` 覆盖“构建路径含 `panic` 不失败”和“真实 Guest panic 必须失败”；Reader mutation 要求参考值只能出现在 `Reference Catalog`，动态指标显示 `n/a`。 | 当前修改后的 seeded AgentOS 与 Reader 定向 E2E 已在本地通过，Host/mutation 与字段解析 probe 也通过；这些仍未绑定 clean C，完整 `full-verify` 和 E3 待重校准及 bundle。示例中的 `expected_*`、`reference_ready` 只是输入目录，不是运行通过证据。 |
...
| H-17 | P2：系统偏重，模块拆分只完成一半 | `agent.c` 收敛为无可写状态 facade；身份、Context、IPC、生命周期、tool protocol、观测和 metadata 分 owner。版本化 registry 精确登记 owner、依赖、SCC、逐模块 LOC/BSS，并额外限制全内核源码、ELF/raw image、runtime text/data/BSS、`struct proc`、线程/boot 栈和 metadata control-plane 聚合 source/text/BSS，防止靠搬文件或提高单项上限隐藏增长。 | `make ci-check` 同时执行 budget selftest、模块边界、生产对象构建和栈调用图检查。此前冻结前 Host 快照为：source `49613/49628` 行、raw image `335880/344073` B、runtime text `334948/339957` B、BSS `6271992/6516729` B、`struct proc 26448/27233` B、线程栈 `15072/16384` B、boot 栈 `10560/65536` B；metadata loaded text `76450/77896` B、BSS `1118596/1118596` B。预算 profile、精确 allowlist 与 metadata aggregate source/text/BSS 上限已经锁定，不再处于“待校准”状态。 | 这些数值只证明其对应的冻结前 Host 快照，不等于当前候选 C 的 canonical 指标。18-case 三轮校准已在冻结提交 `31d4ddf53695` 完成；当前候选仍须重新完成 `ci-check`、干净 `full-verify` 和 C→E evidence bundle，最终指标只能从所选 bundle 读取。 |

## 补充残余项

并行审计在原 17 项终审之外发现一个 P1：Agent metadata catalog 原先只有固定数组上限，且 scanner 维护了第二套全表匹配逻辑。冻结期将 512 槽收敛为 SYSTEM 64 与 4 个 workflow 各 112 的固定分区；ACTIVE、CLOSING、RETIRING 共用 4 个准入槽，RETIRING 在目录回收完成前仍占原槽，PENDING 和 QUARANTINE 也计入本 scope 的 112。workflow inode 账户同步限制为 112；superblock 中 342 inode 只是原始存储保证，不是 Agent metadata tracked 上限。该减法设计不再跨 scope 借用，不需要全局 union/max、catalog resource kind、backing lease 或 metadata envelope 账本。full boot 重验固定边界，live scoped reload 把 lifecycle id/generation 绑定到 plan 并在 prepare/apply 重验；容量、身份变化、键冲突和不确定提交保留各自错误状态。action status batch 的独立上限仍为 112，溢出在 mutation 前返回可恢复错误。scanner 现在构造 physical/logical path 与完整 `dev/inum/incarnation` selector，只调用唯一的 `agent_metadata_catalog_resolve()`：唯一路径才能借用隐藏记录，identity-only 或 split-key 冲突要求重试，同名新 incarnation 分配新 FID；精确 14 字节目录名保持合法。独立的 `_find`/`_find_scan` 全表循环被删除，预取也复用同一 resolver。当前 Host 合同数量须由候选上的实际 checker 输出更新；这些仍是 E1，最终 AgentScope QEMU、恢复路径和 bundle 证据待当前候选 C 复验。
...
当前候选因性能输入变化已回到 provisional，冻结后须在 clean detached worktree 串行取得至少
三轮完整样本。剩余 provenance 边界是 release registry 尚未自动重放校准目录中的原始 timing、
Guest/runner 语义、环境与哈希；这些材料仍需随本轮校准保存并人工审查，不能宣称远端
attestation 或 E3。后续应把重放并入现有 evidence registry，而不是另造平行 delivery 状态机。
