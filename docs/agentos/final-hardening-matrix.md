...
| H-06 | P1：最终证据没有随仓库交付 | 采用“代码提交 C + 证据提交 E”模型：在干净 C 的 detached worktree 执行唯一 `make full-verify`，以 schema v6 manifest/summary、共享语义 registry、原始日志、环境、指标、CSV、图和全文件校验和绑定 C。双目标步骤把每侧完整 Guest 状态打成确定性 ZIP，Host run receipt 作为独立 raw sidecar；E 必须是以 C 为唯一父提交的直接子提交，只新增单一 release 目录的普通文件并对 `evidence/releases/INDEX.md` 作精确单行追加；manifest 以 `SELF` 绑定实际承载包的 E。 | `python3 scripts/capture-final-evidence.py collect ...`、`verify`、`python3 host_tools/evidence_delivery_contract.py verify-committed ...` 和 `make evidence-capture-selftest`。mutation 合同安全解包并重放完整双目标状态，拒绝 Host receipt 混入 Guest ZIP、selected source 字节换绑、passed-only 日志、缺失/额外语义产物、非直接父子关系、第二父提交、C 中预存 release、E 中任意额外改动、非普通文件及非精确索引追加。远端另要求 1 Host + 8 QEMU job 的 API 身份、唯一 trace marker、安全 ZIP、逐文件 attestation 和离线语义复验。 | 采集、语义 registry、严格 C→E 交付及远端 attestation 的 Host/mutation 合同已达到 E1；具体 E3/E4 不由本行冻结，而由 `INDEX.md` 选中包的 manifest 和离线复验结果决定。本地包可在 `remote_ci.status=not-attached` 时达到 E3；只有同一 C 的九个远端 attestation 全部有效时才达到 E4。 |
...
冻结提交 `814021ab9dac` 曾在 clean detached worktree 串行取得三轮完整样本，并以当时的源码/合同
指纹形成历史 calibrated 基线。新候选不得复用与自身指纹不一致的基线；当前策略与实际门禁
结果见 `ci/kernel-budgets.json` 和 [test-record.md](test-record.md)。剩余 provenance 边界是 release registry 尚未自动重放校准目录中的原始
