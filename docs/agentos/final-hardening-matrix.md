...
校准证据仍有一个受控的 P1 provenance 边界：`calibration_samples` 只被预算 checker 自动读取
sample id 与总秒数，尚未由 release registry 自动重放原始 timing、Guest/runner 语义、被测提交
和环境。冻结提交 `31d4ddf53695` 的三轮已在 clean detached worktree 串行完成；仓库保留原始
timing、确定性压缩日志、环境、退出状态和逐文件哈希，并由
`evidence/calibrations/31d4ddf53695/manifest.json` 连接这些材料。校准因此可经人工审查恢复
`calibrated_full_suite`，但不能宣称远端 attestation 或 E3。后续应把该 manifest 的重放合并进
现有 evidence registry，而不是另造一套平行 delivery 状态机。
