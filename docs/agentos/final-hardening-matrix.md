...
冻结提交 `04c1e6652324` 已在 clean detached worktree 串行取得三轮完整样本，并以当前受管
源码/合同指纹形成 `calibrated_full_suite` 基线；包内 71 个文件及逐执行 attestation 可离线重放。
新候选不得复用与自身指纹不一致的基线；当前策略与实际门禁结果见 `ci/kernel-budgets.json` 和
