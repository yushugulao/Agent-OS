...
| G-4 | 提供用户态测试程序 | 已验证 | `agentfinal_ucore`、`agentfs_ucore`、`agentscan_ucore`、`agentloop_ucore`、`agentsched_ucore`、`agentconflict_ucore`、`agentllm_ucore`、`agentbench_ucore`、`labbench_ucore`、`labdemo_ucore`、`agentsecurity_ucore`、`agenttrust_ucore`、`agentvfs_ucore`、`agentscope_ucore`、`iobudget_ucore`、`usersafety_ucore`、`fsenospc_ucore`、`fsquota_ucore`、`fspquota_ucore`、`procreap_ucore`、`procreap_agent_ucore`、`syscallfair_ucore`、`fileresource_ucore`、`threadresource_ucore` | 本次线程改动后已通过线程资源专项、完整 16 项 Agent 脚本、单独 `agentsched_ucore` 和双目标进程回收/syscall 公平性/filepool 脚本；完整 Agent 轮墙钟约 321s，证据边界见 [verification.md](verification.md) 和 [test-record.md](test-record.md) |
