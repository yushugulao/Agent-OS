...
| G-2 | 系统可在 QEMU 上运行 | 已验证 | `Makefile`、`nfs/fs.img` | `scripts/run-agent-tests.sh`、`scripts/run-fs-enospc-tests.sh`、`scripts/run-proc-reap-tests.sh`；`scripts/run-file-resource-tests.sh` 已输出 AgentOS、baseline 和 both targets passed |
| G-3 | 提供内核代码 | 已验证 | `os/` | Git 仓库源码 |
| G-4 | 提供用户态测试程序 | 已验证 | `agentfinal_ucore`、`agentfs_ucore`、`agentscan_ucore`、`agentloop_ucore`、`agentsched_ucore`、`agentconflict_ucore`、`agentllm_ucore`、`agentbench_ucore`、`labbench_ucore`、`labdemo_ucore`、`agentsecurity_ucore`、`agenttrust_ucore`、`agentvfs_ucore`、`agentscope_ucore`、`usersafety_ucore`、`fsenospc_ucore`、`fsquota_ucore`、`fspquota_ucore`、`procreap_ucore`、`procreap_agent_ucore`、`syscallfair_ucore`、`fileresource_ucore` | `scripts/run-agent-tests.sh` 15/15、ENOSPC/持久主体与进程回收专项独立通过；syscall 公平性基础轮通过，last-syscall 终审契约待标准工具链复测；`fileresource_ucore` 已在 64/48/16/16 配置下双目标通过；证据见 [verification.md](verification.md) 和 [test-record.md](test-record.md) |
...
S-16 验证的是已识别的可扩展长路径和相应生命周期协议，不表示已经穷尽任意 syscall 实现。固定上界的目录扫描，以及只允许可信 Agent 到达的 metadata raw I/O，仍是后续接入统一 checkpoint 或增加专项压力证据的残余覆盖。S-18 把全局 filepool 的唯一对象槽纳入资源域和系统保留机制；运行证据必须以独立双目标 `file-resource-test` 为准，不能用既有的每进程 fd 槽回滚用例替代。
...
| T4-26 | metadata 内存表、索引、inode sidecar、依赖和双 bank 提交形成统一并发事务，查询不得观察半提交状态 | 已验证 | 可睡眠可重入 metadata transaction gate；inactive bank 完成后的安全协作调度点；owner 清空与专属 wait queue 广播处于同一关中断临界区；所有 waiter 重新检查条件，不传递所有权 baton；VFS/scheduler external try-lock；按 scope dirty/durable generation 与分片扫描协调；每进程等待计数提供争用可观测性 | `agentscope_ucore: metadata_transactions=1` 已验证三个同 scope Orchestrator 中至少两个实际等待、全部完成并重载得到 12 条完整记录 |
| T4-27 | PUBLIC 持久存储计费必须跨完整进程域退出和磁盘重挂载保持，删除后才能归还并复用容量 | 已验证 | 安装级 PUBLIC principal 2、VFS 独立计费凭据、qmap/dinode 持久 owner、挂载孤儿清扫与 PUBLIC 用量重建、SYSTEM 赞助对象首次修改时的 qmap-first 整体接管、版本化 superblock 契约 | `fspquota_ucore` 双目标 crash/seed/verify 三阶段同镜像回归及 runner 顺序/计数断言均通过 |
| T4-28 | 低权限 workflow 的高频微小文件变化不得同步放大全局 metadata I/O，也不得阻断另一 workflow 的查询服务 | 已验证 | inode incarnation sidecar 先发布 size/generation/tick；只有 `PERSIST` 记录进入按 scope dirty/durable/request/coalesced/commit 记账，volatile 内容变化不触发 bank；固定一秒非滑动合并窗口；checkpoint 成功或失败后都至少休息该次尝试耗时四倍；扫描请求也采用非滑动 deadline，完整扫描/失败后按 `max(20 tick, 4 * 扫描耗时)` 休整，满表未绑定对象不能制造完成即重启；到期写回先于扫描；capture/complete 防止并发新写入被误报为 durable；查询缓存代数按 scope 隔离且 SYSTEM 变化统一失效 | `metadata_write_coalescing=1 writes=<at-least-128> commits=<bounded>`、`metadata_cross_scope_progress=1 queries=32 latency_ms=<at-most-5000>`、`metadata_final_consistency=1`、`metadata_volatile_no_writeback=1 writes=32`、`metadata_scan_pressure_bounded=1`；失败退避由统一 deadline helper 的源码与构建检查覆盖 |
...
任务一至三已有增强实现和既有测试证据；本次进一步把 capability、VFS 凭据、Context cause/span、audit 和对象表统一绑定到 kernel-issued workflow scope。任务四的真实对象身份现在是 scope 与 `dev + inum + incarnation` 的组合，metadata、依赖、动作、租约、版本、缓存和预取均按 scope 分区；任务五的 IPC、wait control、可信 span、timeline 和账本也只在同一 active scope 内生效。物理审计容量仍为 512，但不是任意 orchestrator 可见的全局池：最多 4 个 scope 各保证 128 条，low/high 各 64；low principal 上限16，high active principal 上限8且不能被其他主体淘汰。`agentscope_ucore` 与新增 `agentsecurity_ucore` 断言已由 2026-07-22 完整 Agent QEMU 回归验证，当前脚本 15/15 通过；聚合 `full-verify` 状态仍单独记录。通用稳定性机制还覆盖 syscall 输入防护、文件系统 ENOSPC、guarded kernel stack、协作退出、`child_record`、孤儿回收、普通 96/保留 32 进程槽，以及 `NINODE=2048` 下的 workflow inode/block 保证；全局 filepool 的资源域配额、ordinary 水位和受控保留已由 `make file-resource-test` 在双目标通过。当前交付范围不包含多级目录递归扫描、复杂策略语言和内核直连云端 LLM Gateway。
