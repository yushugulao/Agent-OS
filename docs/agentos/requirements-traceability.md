...
| --- | --- | --- | --- | --- |
| G-1 | 在教学操作系统内核中实现 Agent-OS 功能模块 | 已验证 | `os/agent.c` facade；`os/agent_core.c`、`agent_context.c`、`agent_identity.c`、`agent_ipc.c`、`agent_lifecycle.c`、`agent_metadata*.c`、`agent_observe.c` owner 模块；`os/proc.c` | 当前模块拆分版本在固定 runner 连续三轮完成 16/16，且 `make ci-check` 完整通过 |
| G-2 | 系统可在 QEMU 上运行 | 已验证 | `Makefile`、`nfs/fs.img` | `scripts/run-agent-tests.sh`、`scripts/run-fs-enospc-tests.sh`、`scripts/run-proc-reap-tests.sh`、`scripts/run-syscall-fairness-tests.sh`、`scripts/run-file-resource-tests.sh`、`scripts/run-thread-resource-tests.sh`；各专项的当前或历史证据边界见 [verification.md](verification.md) |
| G-3 | 提供内核代码 | 已验证 | `os/` | Git 仓库源码 |
| G-4 | 提供用户态测试程序 | 已验证 | `agentfinal_ucore`、`agentfs_ucore`、`agentscan_ucore`、`agentloop_ucore`、`agentsched_ucore`、`agentconflict_ucore`、`agentllm_ucore`、`agentbench_ucore`、`labbench_ucore`、`labdemo_ucore`、`agentsecurity_ucore`、`agenttrust_ucore`、`agentvfs_ucore`、`agentscope_ucore`、`iobudget_ucore`、`usersafety_ucore`、`fsenospc_ucore`、`fsquota_ucore`、`fspquota_ucore`、`procreap_ucore`、`procreap_agent_ucore`、`syscallfair_ucore`、`fileresource_ucore`、`threadresource_ucore` | 当前完整 Agent 套件连续三轮 16/16；`agentscope_ucore` 约 93.7s 并通过 PUBLIC lineage marker；五组机制专项通过，边界见 [verification.md](verification.md) 和 [test-record.md](test-record.md) |
...
| G-8 | 内核增长、模块边界和关键运行预算必须可审查并在 CI 中拒绝回退 | 已验证 | `.gitlab-ci.yml`、`ci/kernel-budgets.json`、`scripts/check-kernel-budgets.py`、`scripts/check-agent-module-boundaries.sh`、`make ci-check` | 预算 checker 31 项、通用 QEMU monitor 24 项、生产 profile validator 5 项 fail-closed 单测；12 个注册模块与 9 个 bridge 的受控符号图硬限 SCC=3；runner 输出/记录/诊断有界且 deadline 优先 checkpoint；duration 只接受完整 16-case timing file；bounded/flood-safe 三轮中位时间门为 `255.370930671s/268.14s` |
...
| T1-6 | Agent 退出后 Context 状态、文件、VM、resource account、lifecycle 与内核栈按统一 teardown 顺序释放 | 已验证 | `proc_teardown_run()` 对 Agent 私有清理只调用 phase-aware、幂等的 `agent_proc_teardown()`；其 QUIESCING 撤权、RECLAIMING 释放/清身份、SETTLING 只验 Agent 状态/页账为空，通用 process/thread/file/I/O 账目仍由外层状态机结算；scheduler stack publish/recycle | 当前 Agent、agentscope、proc-reap 与资源专项通过 |
...
| T1-9 | workflow 根退出或可信控制面终止时，整个 lifecycle 谱系立即失权并最终回收；降权 PUBLIC 后代不能逃逸 | 已验证 | controller control id、generation-safe lifecycle key、ACTIVE/CLOSING/RETIRING、按 key 撤销、exec 原子发布和统一 teardown | 当前输出含 `scope_controller_exit_revoke=1 public_lineage=1` 和 `parent passed` |
...
任务一至三已有增强实现；当前授权凭据与不可变 lifecycle key 分离，PUBLIC 降权、fork 和 exec 不再改变终止谱系。进程、线程、文件、存储、缓存、I/O 与 Agent 私有页统一映射到 generation-safe EXEC/STORAGE account，CPU 公平仍由独立调度 domain 实现。`os/agent.c` 已收敛为 facade，Context/身份/IPC/lifecycle/metadata/观测由 owner 模块持有；完整 Agent 状态以 21 页原子计费，detail/attribution 从 PCB 移到 9 页按需 sidecar。phase-aware `agent_proc_teardown()` 只负责 Agent 撤权、状态回收和 Agent 页账验空，通用 process/thread/file/I/O 资源结算由外层 teardown 状态机完成；内核物理栈按 live thread 分配。CI 对 12 个注册模块和 9 个受控符号 bridge 设硬 SCC=3，并用 31 项预算、24 项通用 runner 与 5 项生产 profile validator 自测防止门禁被静默放宽。bounded/flood-safe runner 当前三轮 16/16、agentscope 和五组机制专项均已通过；时间中位基线/上限已收紧为 `255.370930671s/268.14s`。`make full-verify` 尚未运行，故不宣称聚合全绿。
