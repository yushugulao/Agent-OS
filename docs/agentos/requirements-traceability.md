...
| G-4 | 提供用户态测试程序 | 已验证 | `agentfinal_ucore`、`agentfs_ucore`、`agentscan_ucore`、`agentloop_ucore`、`agentsched_ucore`、`agentconflict_ucore`、`agentllm_ucore`、`agentbench_ucore`、`labbench_ucore`、`labdemo_ucore`、`agentsecurity_ucore`、`agenttrust_ucore`、`agentvfs_ucore`、`agentscope_ucore`、`usersafety_ucore`、`fsenospc_ucore`、`fsquota_ucore`、`fspquota_ucore`、`procreap_ucore`、`procreap_agent_ucore`、`syscallfair_ucore` | `scripts/run-agent-tests.sh` 15/15、ENOSPC/持久主体与进程回收专项独立通过；syscall 公平性基础轮通过，last-syscall 终审契约待标准工具链复测；证据见 [verification.md](verification.md) 和 [test-record.md](test-record.md) |
