...
`.agentmeta` 和 `.agentmeta1` 是标记为 `KERNEL_PRIVATE` 的 Agent 子系统内部后端。普通文件 syscall 直接读取、创建、修改或删除任一 bank 都会返回 `-1`；Agent 子系统内部 helper 使用内核凭据读写。加载时按 header 声明的逻辑长度验证版本、generation、payload hash、slot 范围、中央 catalog 硬边界和 scope 内唯一性，物理文件可以保留更长的已分配高水位。提交先向目标 bank 写无效 header，再按固定 1 KiB segment 与已验证的内存 bank shadow 做字节比较，只写并逐段读回核对变化 payload；整体摘要一致后才发布有效 header，header 回读一致后才切换 active generation。新的 primary 完整验证后，状态机才允许用同一不可变快照更新旧 bank 作为 mirror。缩短快照不在 checkpoint 中同步 truncate，而是复用旧尾块；primary 验证前失败保留旧 active bank，mirror 阶段失败仍保留新 primary。payload hash 是一致性摘要，不是密码学认证。

启动时，可信 bank 不再等到首个 Agent 查询才懒加载。文件系统和 timer 初始化后，内核先完成 `agent_storage_init()`，再启动运行时 I/O policy 和首个用户进程。mkfs 预装两个完整预分配、尾部清零的 KERNEL_PRIVATE/SYSTEM bank；运行时只在 inactive shadow 中准备恢复计划，取得 mutation fence 并重验 identity、generation 与 hash 后才发布。暂态 `BUSY/EIO/INTERRUPTED` 有界重试，确认的身份或 hash 冲突 fail closed；任一有效 bank 可以修复另一副本，没有有效 bank 时拒绝继续使用 metadata。`metadata-recovery` 以版本化 Host parser 和认证 QEMU 终止/重启检查 COW phase、明确 I/O 失败和副本修复。该故障模型覆盖设备 flush/durable-barrier 契约，不把宿主 `fsync()`、控制器易失缓存、整机断电或永久介质故障描述为已证明的物理掉电原子性。
...
显式持久 metadata set/delete、reload 和 scope retirement 还会同步进入单调 ticket 的 FIFO submit lane，并建立不可替换的 COW job；该语义保证有序接纳，不保证调用返回时 mirror 已完成。密封快照期间 catalog 可继续合并修改；同 owner 的下一次持久提交先在有界预算内完成旧 job，回到 idle 后再捕获新目标，不能跨 owner 或接管待重启任务。条件不满足时，中断从失败检查一直保持关闭，直到事务门释放且线程已经进入 submit condition queue，避免 unlock-to-sleep 丢失唤醒。持久化在预算边界暂时释放事务门时，immutable job 保持同一 `job_id`，后来提交者不能替换。metadata exact-read 在文件系统原子段之外偿还 I/O 预算，并从正数短读前缀继续；临时 interruption 不会被记成 bank corruption。scanner 仍独立使用 `max(20 tick, 4 * 扫描耗时)` 自适应 cooldown，不能与 checkpoint 的固定窗口混为一谈。
...
正式证据位于 `evidence/releases/<bundle>/metrics/file-query-benchmark.{csv,json}`。普通双目标运行在新建的私有随机目录中生成 Guest 日志、manifest、CSV 和最后发布的 `measurement-set.json`；正式采集不会复用该目录，而是在自己的私有 staging 目录重新运行并封存到 bundle 的 `logs/raw/`。原始输出见 [正式证据索引](../../evidence/releases/INDEX.md)。这些测量用于确认当前系统同时具备：
