...
Agent suite duration 只累计各 QEMU case 的 monotonic 运行时间，不包含编译。固定 runner 上 `bounded-runner-final-01/02/03` 的历史 16/16、`31d4ddf53695`、`814021ab9dac` 和 `04c1e6652324` 的三轮 18/18 都只证明各自源码。当前配置为 `calibrated_full_suite`，绑定 `a9e7c67feda5`；三轮 18/18 总时长为 `269.1409306s`、`271.32236290000003s`、`281.8869957s`，中位基线 `271.32236290000003s`，上限 `284.889s`。该包仍只是未签名本地 E3 校准证据，不是当前 release bundle、GitLab CI 或 E4 attestation。
...
| `iobudget_ucore` | syscall 544 ABI v5 sized-copy、稳定 PUBLIC/workflow owner、NORMAL/CONTROL class、owner/shared/device lease 上界、工作保守型空闲容量借用、线程退出 lease 回收、scheduler 内核态中断交付、fault teardown 清理归因/debt 结算、完成归因、PUBLIC cache/速率压力、workflow cache floor 与压力下写入进展。 | 历史轮不能替代当前候选；本次发布结果只从 C 对应 bundle 读取 |
