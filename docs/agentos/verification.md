...
- profile v6 聚合 Agent suite、双目标与独立资源/恢复/故障 runner。动态是否通过只由 [正式证据索引](../../evidence/releases/INDEX.md) 指向的已验证 bundle 判定。
