...
| `agentfs_ucore` | 真实 inode 绑定、metadata 双 bank、属性查询、每次真实执行的扫描/索引查询、内容摘要、有界去重预取、文件删除清理、字段驱动批量 action 状态维护、依赖图稳定性与 generation 单调性、metadata 工作预算和交接端点生命周期。 | 重复查询不得出现内核 `CACHE_HIT`；`metadata_action attempts=<次数> preemptions=<让出次数> graph_edges=<边数> generation_delta=<增量>`、`prefetch_hints=1 bounded=1 count=2 preemptions=<次数>`、`handoff_target_exit=1 endpoint_reuse=1 preemptions=<次数> ... clean=1`、`agentfs_ucore: parent passed` |
...
| `iobudget_ucore` | syscall 544 ABI v6 sized-copy、稳定 PUBLIC/workflow owner、NORMAL/CONTROL class、owner/shared/device lease 上界、请求内物理传输批量结算、工作保守型空闲容量借用、线程退出 lease 回收、scheduler 内核态中断交付、fault teardown 清理归因/debt 结算、完成归因、PUBLIC cache/速率压力、workflow cache floor 与压力下写入进展。 | `work_conserving=shared-loan shared=<实际传输数> refills=<次数> throttles=0`；正式数值只从同一冻结提交的 bundle 读取 |
