...
Agent suite duration 只累计各 QEMU case 的 monotonic 运行时间，不包含编译。固定 runner 上 `bounded-runner-final-01/02/03` 的历史 16/16、`31d4ddf53695`、`814021ab9dac` 和 `04c1e6652324` 的三轮 18/18 都只证明各自源码。当前受管输入已变化，配置为 `provisional_requires_full_suite`；冻结提交后必须重新取得三轮完整样本，且不得夹带旧 baseline、limit、samples 或 fingerprint。历史包仅是未签名本地 E3 校准证据，不是当前 release bundle、GitLab CI 或 E4 attestation。
