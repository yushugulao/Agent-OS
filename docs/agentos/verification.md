...
Agent suite duration 只累计各 QEMU case 的 monotonic 运行时间，不包含编译。固定 runner 上 `bounded-runner-final-01/02/03` 的 `261.343281873s`、`237.948978492s`、`255.370930671s` 来自历史 16/16，不能证明当前 18-case 套件。`ci/kernel-budgets.json` 因此处于 `provisional_requires_full_suite`，不携带旧 baseline、limit 或 sample；普通 full-suite/CI 会在 QEMU 前 fail closed，定向开发用例仍可运行。发布前必须在同一 `agentos-qemu-calibrated` runner 用校准模式取得至少三轮完整 timing file，经审查写入新中位基线、上限和 sample 后才能恢复 calibrated；更换硬件、虚拟化层、QEMU 或 case 集合同样必须重新进入 provisional。
...
- 18-case duration 仍须在同一 `agentos-qemu-calibrated` runner 上完成至少三轮完整 calibration timing file。旧 16/16 的 `255.370930671s` 中位数和 `268.14s` 上限只作历史记录，不能填入当前 18-case 结果；
