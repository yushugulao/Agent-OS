...
| `rp_agentos_mainflow` | AgentOS main-flow kernel facts | backend, consistency, metrics, compare, test suite | enhanced-target RUN-042 facts appended by the real platform stages: trusted Context, indexed metadata query, Agent event notification, recovery tools, permission denial, timeline observation, ledger/provenance observation, workbench file verification, package provenance, and real-task report/audit Context |
...
| `rp_analysisres` | analysis results service | test suite, compare, Host reader, orchestrator | analysis plan/run/table/statistic/figure/interpretation summary for RUN-042 and the manual QC run |
| `rp_anplan` | analysis results service | test suite, compare, Host reader | materialized analysis plan for treatment-response evaluation |
| `rp_anrun` | analysis results service | test suite, compare, Host reader | workflow and manual analysis run records |
| `rp_resulttbl` | analysis results service | test suite, compare, Host reader | result table exports for gene summary and manual QC |
| `rp_statres` | analysis results service | test suite, compare, Host reader | statistical result records for treatment-vs-control and manual checks |
| `rp_anfig` | analysis results service | test suite, compare, Host reader | figure records linked to analysis runs |
| `rp_interp` | analysis results service | test suite, compare, Host reader | interpretation records with reviewer-ready conclusions |
| `rp_decsupport` | decision support service | test suite, compare, Host reader, orchestrator | final architecture decision summary for the same RUN-042 workflow and AgentOS-readiness evidence |
| `rp_decopt` | decision support service | test suite, compare, Host reader | candidate architecture options: user-space baseline, AgentOS-uCore hybrid, and full kernel LLM path |
| `rp_deccrit` | decision support service | test suite, compare, Host reader | weighted decision criteria for OS value, reproducibility, performance signal, migration effort, and reviewer clarity |
| `rp_decscore` | decision support service | test suite, compare, Host reader | per-option scored criteria with rationale records |
| `rp_decpacket` | decision support service | test suite, compare, Host reader | review packet linking the selected option to backend, study, LLM relay, package, and release evidence |
| `rp_usable` | usable research service | test suite, compare, Host reader, orchestrator | research workbench entry summary with templates, datasets, library sources, DAG stages, queues, handoff packages, and delivery exports |
| `rp_usabletpl` | usable research service | test suite, compare, Host reader | reusable workbench templates for workspace, analysis, and review handoff |
| `rp_usableds` | usable research service | test suite, compare, Host reader | reusable dataset handles, including the Palmer Penguins dataset used by the real-task workflow |
| `rp_usablelib` | usable research service | test suite, compare, Host reader | reusable library/source records for current evidence and citation workflows |
| `rp_usabledag` | usable research service | test suite, compare, Host reader | nine-stage research DAG from intake through package export |
| `rp_usableops` | usable research service | test suite, compare, Host reader | plan queue, action queue, handoff packages, runbook, timeline, and manifest export records |
| `rp_usableproj` | usable project lifecycle service | test suite, compare, Host reader, orchestrator | configuration, startup guide, platform doctor, scaffold, launch, package, and operations digest summary |
| `rp_usableboot` | usable project lifecycle service | test suite, compare, Host reader | required configuration, startup guide, and platform doctor checks for the usable research path |
| `rp_usablescaf` | usable project lifecycle service | test suite, compare, Host reader | scaffold templates and generated starter project files |
| `rp_usablelaunch` | usable project lifecycle service | test suite, compare, Host reader | launched projects and operations digest records |
| `rp_usablepack` | usable project lifecycle service | test suite, compare, Host reader | project bundle, study reproduction package, package intake, and package action records |
...
| `rp_stdesign` | statistical design service | test suite, compare, Host reader, orchestrator | RUN-042 primary study design with endpoint, comparison, effect size, target power, and reviewer decision |
| `rp_power` | statistical design service | test suite, compare, Host reader, orchestrator | power-analysis record with required sample size, available group size, achieved-power estimate, and status |
| `rp_random` | statistical design service | test suite, compare, Host reader, orchestrator | deterministic control/treatment randomization plan with assignments, strata, seed, and balance status |
| `rp_blind` | statistical design service | test suite, compare, Host reader, orchestrator | blinding-role check with blinded and unblinded roles plus leak count |
| `rp_streview` | statistical design service | test suite, compare, Host reader, orchestrator | statistical review record linking evidence, export handle, checksum, and decision |
| `rp_modelreg` | model registry service | test suite, compare, Host reader, orchestrator | registered research Agent model/template with owner, task, tags, type, and readiness status |
| `rp_modelver` | model registry service | test suite, compare, Host reader, orchestrator | version record linking model card, training run, produced artifacts, and evaluation metrics |
| `rp_modeleval` | model registry service | test suite, compare, Host reader, orchestrator | evaluation record for the registered version, evidence coverage, report status, prompt score, and output report |
| `rp_modeldep` | model registry service | test suite, compare, Host reader, orchestrator | deployment-candidate record with model-card, evaluation, provider, and secret-policy checks |
| `rp_modelserve` | model registry service | test suite, compare, Host reader, orchestrator | offline serving-readiness check for the registered template provider |
| `rp_sysreview` | systematic review service | test suite, compare, Host reader, orchestrator | registered review protocol, research question, PICO-style fields, owner, and readiness status |
| `rp_syssearch` | systematic review service | test suite, compare, Host reader, orchestrator | local literature-search strategy, query, filters, result count, and source |
| `rp_sysscreen` | systematic review service | test suite, compare, Host reader, orchestrator | title/abstract and full-text screening decisions with include/exclude counts |
| `rp_sysextract` | systematic review service | test suite, compare, Host reader, orchestrator | extracted evidence fields and risk-of-bias summary for included records |
| `rp_syssynth` | systematic review service | test suite, compare, Host reader, orchestrator | synthesis conclusion, confidence, limitations, and included-paper count |
| `rp_sysprisma` | systematic review service | test suite, compare, Host reader, orchestrator | PRISMA-style identified, screened, excluded, and included counts |
| `rp_expsched` | experiment scheduling service | test suite, compare, Host reader, orchestrator | RUN-042 controlled lab execution schedule with owner, run window, project, run id, and approval status |
| `rp_schedtask` | experiment scheduling service | test suite, compare, Host reader, orchestrator | verify-resources, library-prep, and SOP-review task rows with assignee, dependency, target, and evidence links |
| `rp_schedbook` | experiment scheduling service | test suite, compare, Host reader, orchestrator | sequencer, lab-tech, QA-reviewer, and overlapping sequencer bookings with resource, task, time window, and conflict status |
| `rp_schedconf` | experiment scheduling service | test suite, compare, Host reader, orchestrator | detected sequencer overlap with severity, affected booking, resource, status, and resolution hint |
| `rp_schedexec` | experiment scheduling service | test suite, compare, Host reader, orchestrator | completed execution records for resource verification and library preparation with evidence and notes |
| `rp_traincomp` | training compliance service | test suite, compare, Host reader, orchestrator | training-compliance summary for RUN-042 with requirements, records, competency assessments, authorizations, resolved gap count, and readiness status |
| `rp_trainreq` | training compliance service | test suite, compare, Host reader, orchestrator | four role-specific training requirements derived from the RUN-042 schedule and resource plan |
| `rp_trainrec` | training compliance service | test suite, compare, Host reader, orchestrator | training evidence rows for lab-tech, auditor, and QA lead, including the resolved SOP-deviation record |
| `rp_trainassess` | training compliance service | test suite, compare, Host reader, orchestrator | competency assessments and pass decisions for required RUN-042 roles and topics |
| `rp_trainauth` | training compliance service | test suite, compare, Host reader, orchestrator | active role authorizations for lab-tech, auditor, and QA lead on project `lab-gene-x` |
| `rp_traingap` | training compliance service | test suite, compare, Host reader, orchestrator | resolved training gap tying the QA-lead SOP-deviation requirement back to concrete evidence |
