...
本项目基于 xv6-riscv，在教学操作系统内核中加入面向 AI Agent 的内核支持层，使内核能够识别 Agent 进程、提供结构化内核工具调用、维护 Agent 多轮调用上下文路径，并为后续 Agent Loop、文件系统查询优化和综合演示保留扩展边界。
...
| 优先级 | 质量目标 | 可验证方式 |
| ---: | --- | --- |
| 1 | 稳定性 | `agentfinal`、`agentbench` 均通过且无 kernel panic |
...
| 5 | 可扩展性 | Context Path、mailbox、Loop 状态、工具表可继续扩展到任务四至六 |
...
| 文件系统查询优化、索引、语义检索 | 未完整实现 |
| Agent Loop 心跳、等待、唤醒 | 仅保留字段和运行状态，不是完整任务五 |
| 综合场景 | 尚待设计 |
...
| 测试驱动验收 | 用 `agentfinal` 做最终功能验证，用 `agentbench` 输出吞吐对比 |
...
| PCB 和生命周期 | `kernel/proc.h`、`kernel/proc.c` | 保存 Agent 元数据，处理 fork/exec/exit/sbrk 边界 |
...
sequenceDiagram
    ...
    A->>C: 更新 latest/head/count/rollback_count
    A-->>U: 返回状态
```
...
| 任务四文件查询尚浅 | 当前只有文件元数据查询 | 增加索引、属性过滤、内容摘要或语义标签 |
| 任务五 Loop 未完整实现 | 目前只有 loop_state 字段和调用时状态变化 | 增加心跳、等待队列、唤醒路径 |
| 综合场景未成型 | 创新展示不足 | 设计系统管理员 Agent 或文件检索 Agent 场景 |
