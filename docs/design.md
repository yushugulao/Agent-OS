...
- Captured host actions are translated by `host_tools/plain_ucore_action_runner.py` into `rp_host_action_queue`, `rp_host_action_plan`, and `rp_host_action_inbox`. When requested, the runner writes the inbox text into a generated user-build header before `rp_orch` starts, while keeping the queue and plan in the host run package. `rp_artifact_ops`, `rp_workflow_runner`, `rp_metrics`, `rp_web_export`, `rp_test_suite`, and `rp_compare_plain` read the seeded inbox text. A seeded research run updates `rp_input`, `rp_runner`, and `rp_agentcmp`; a seeded comparison run updates `rp_runner`, `rp_actionio`, and `rp_agentcmp`.
