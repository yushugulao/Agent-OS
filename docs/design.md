...
| `rp_review_pack` | reviewer evidence package state | Host Web/API export, Host LLM Relay, review dashboard | host-materialized reviewer handoff package with required-file, workflow, artifact, LLM quality, packet guard, human review, revision, delivery, operations, project-space, and workbench handoff checks plus reviewer actions |
...
| `rp_artifact_manifest` | workflow runner | package, consistency, metrics, UI export, compare, orchestrator | generated artifact manifest for input, intermediate, report, chart, stage-log, package-index, and artifact-dossier outputs |
...
| `rp_backend_exec` | backend scenario | compare, orchestrator | backend scenario execution result for executable and planned cases, including content checks, attempts, retry reasons, tick observations, and source/requirement/observation/action/review rows |
