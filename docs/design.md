...
| `rp_agentos_mainflow` | AgentOS main-flow kernel facts | backend, consistency, metrics, compare, test suite | enhanced-target RUN-042 facts appended by the real platform stages: trusted Context, indexed metadata query, Agent event notification, recovery tools, permission denial, timeline observation, and ledger/provenance observation |
