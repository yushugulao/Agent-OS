...
AgentOS target 使用 `rp_agentos_orch` 作为入口。它创建 orchestrator Agent，初始化 `rp_agentos_mainflow`，再运行与 plain target 可比较的科研流程。关键阶段会把内核事实写入 `rp_agentos_mainflow` 和相关 `rp_agentos_*` 文件。
