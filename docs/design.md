...
| `rp_web_routes` | Host Web/API export | test suite, compare, orchestrator | route manifest for fourteen host-rendered GET views and thirty-two POST action entries |
...
| `rp_api_action` | Host Web/API export | test suite, compare, orchestrator | action contract for host workflow run, host workflow export, Host LLM Relay request/response/fallback, AgentCompare run, custom research run, dynamic research submission, reusable source lookup, custom research export, human review, revision-task creation, and revised-run execution |
