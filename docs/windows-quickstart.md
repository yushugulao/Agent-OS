...
`local-e3` 只用于与校准记录逐项一致的本地 MSYS2 E3。`14607e825f06` 的三轮完整
18-case 只属于该历史提交；当前配置为 `provisional_requires_full_suite`，必须在最终冻结提交上
重新完成三轮校准后才可启用 `local-e3`。受管输入或 profile 漂移时，doctor/full-verify 会在
