...
The same check verifies `/api/state/rp_review_pack`, the `review_pack=ready` AgentCompare record, host relay quality, backend evidence, backend action/review rows, operations/workbench/project summaries appended to the reviewer evidence package, the operations report narrative derived from `rp_runner`, `rp_package`, and `rp_review_pack`, and the related Host action trace rendered from `host-actions.jsonl`; `run.html`, `compare.html`, and `review.html` must include the Run Action Trace, Compare Action Trace, Review Evidence Pack, Review Backend Evidence, Review Backend Actions, Review Pack Bridges, Review Operations Summary, Review Workbench Summary, Review Project Summary, Operations Report Narrative, and Review Action Trace tables.
