...
The same check verifies `/api/state/rp_review_pack`, the `review_pack=ready` AgentCompare record, host relay quality and backend evidence appended to the reviewer evidence package, and the Review Evidence Pack, Review Backend Evidence, and Review Pack Bridges tables in `review.html`.
