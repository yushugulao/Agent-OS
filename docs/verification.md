...
- user-space test suite with 122 checks over catalog, data pipeline, bio services, lab resource services, publication services, knowledge services, runtime services, workflow, artifact operations, workflow runner files, Agent collaboration, UI export, Host Web/API export files, file-backed POST action records, LLM relay, AgentCompare, and consistency records.
