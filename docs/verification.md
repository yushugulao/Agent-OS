...
The same check verifies `/api/state/rp_review_pack`, the `review_pack=ready` AgentCompare record, host relay quality, backend evidence, and backend action/review rows appended to the reviewer evidence package, and the Review Evidence Pack, Review Backend Evidence, Review Backend Actions, and Review Pack Bridges tables in `review.html`.
