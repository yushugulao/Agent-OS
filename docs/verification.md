...
The same check verifies `/api/state/rp_review_pack`, the `review_pack=ready` AgentCompare record, host relay quality, backend evidence, backend action/review rows, and operations/workbench/project summaries appended to the reviewer evidence package, and the Review Evidence Pack, Review Backend Evidence, Review Backend Actions, Review Pack Bridges, Review Operations Summary, Review Workbench Summary, and Review Project Summary tables in `review.html`.
