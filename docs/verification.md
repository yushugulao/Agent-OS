...
The end-to-end reader check uses the real action runner and QEMU path. It starts the local HTTP handler, posts `/actions/research/run`, waits for `rp_orch`, extracts generated `rp_*` files from `nfs/fs-copy.img`, then verifies `/api/live`, `/api/state/rp_input`, `/api/state/rp_runner`, `/api/state/rp_host_run_result`, `run.html`, and `actions.html`.
