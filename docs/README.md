...
本目录是评审阅读入口。文档结构参考操作系统和软件架构领域常见做法：README 负责快速运行，设计文档负责解释架构和关键决策，API/ABI 文档负责说明用户态与内核边界，验证文档负责给出可复现证据。
...
| 7 | [testing-details.md](testing-details.md) | `agentfinal` 和 `agentbench` 的逐项测试说明 |
...
| [xv6-riscv-README](xv6-riscv-README) | xv6-riscv 原始说明 |
...
- 用户态/内核态边界和结构体布局以 [api.md](api.md) 为准。
...
- `test-record.md` 保留当前输出摘要，不替代验证结论。
