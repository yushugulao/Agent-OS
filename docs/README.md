...
本目录是评审阅读入口。文档结构参考操作系统和软件架构领域常见做法：README 负责快速运行，设计文档负责解释架构和关键决策，API/ABI 文档负责说明用户态与内核接口，验证文档负责给出可复现证据。
...
| 7 | [testing-details.md](testing-details.md) | `labdemo`、`labbench`、`agentfinal` 和 `agentbench` 的逐项测试说明 |
...
- 用户态/内核态接口和结构体布局以 [api.md](api.md) 为准。
