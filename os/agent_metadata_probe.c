...
static void
agent_metadata_probe_new_epoch(void)
{
...
void
agent_metadata_probe_init(void)
{
...
void
agent_metadata_probe_reset(void)
{
...
static void
agent_metadata_probe_bind(const struct agent_metadata_probe_key *key)
{
	if (probe.epoch != 0 &&
...
static int
agent_metadata_probe_fault(int bank, int allowed)
{
	return agent_metadata_recovery_test_fault(bank, allowed);
}

static int
agent_metadata_probe_open(int bank, struct inode **result)
{
	struct inode *ip;
	...
	*result = 0;
	ip = namei_scope_status(agent_meta_store_io_name(bank),
				VFS_POLICY_KERNEL_PRIVATE, VFS_SCOPE_NONE, &status);
	if (ip == 0)
	...
	if (inode_status < 0) {
		...
		return inode_status == VIRTIO_DISK_ERR_BUSY ?
			       AGENT_META_BANK_BUSY : AGENT_META_BANK_IO;
	}
...
static int
agent_metadata_probe_piece(struct inode *ip, const struct vfs_cred *cred,
			   char *dst, uint file_offset, uint length)
{
...
static int
agent_metadata_probe_header_valid(struct agent_meta_store *store,
				  uint64 inode_size)
{
...
static int
agent_metadata_probe_validate(struct agent_meta_store *store,
...
static int
agent_metadata_probe_read(const struct agent_metadata_probe_key *key, int bank,
			  int confirm, struct agent_meta_store *store,
			  uint64 *generation, uint64 *payload_hash,
			  int *migration, int allow_fault)
{
	...
	if (ip->size < sizeof(store->header)) {
		...
		if (!confirm) {
			memset(&probe.cursor, 0, sizeof(probe.cursor));
			probe.summary[bank].classified = 1;
			probe.summary[bank].status = AGENT_META_BANK_UNCOMMITTED;
		} else
			agent_metadata_probe_new_epoch();
		return AGENT_META_BANK_UNCOMMITTED;
	}
	...
	if (probe.cursor.phase == AGENT_META_PROBE_HEADER) {
		status = agent_metadata_probe_piece(ip, &cred,
			(char *)&store->header, 0, sizeof(store->header));
		if (status != AGENT_META_BANK_VALID)
			goto out;
...
out:
	...
	if (status == AGENT_META_BANK_INTERRUPTED ||
	    (!key->resumable && status < 0)) {
		agent_metadata_probe_new_epoch();
		return status == AGENT_META_BANK_PROGRESS ?
			       AGENT_META_BANK_BUSY : status;
	}
	return status;
out_reset:
	iput(ip);
...
int
agent_metadata_probe_summary(const struct agent_metadata_probe_key *key,
		int bank, struct agent_meta_store *store, uint64 *generation,
		uint64 *payload_hash, int *migration, int allow_fault)
{
	int status;
	...
	if (key == 0 || store == 0 || generation == 0 || payload_hash == 0 ||
		...
		return AGENT_META_BANK_CORRUPT;
	status = agent_metadata_probe_read(key, bank, 0, store, generation,
				   payload_hash, migration, allow_fault);
	if (!key->resumable && status < 0) {
		agent_metadata_probe_reset();
		if (status == AGENT_META_BANK_PROGRESS)
			status = AGENT_META_BANK_BUSY;
	}
	return status;
}

int
agent_metadata_probe_confirm(const struct agent_metadata_probe_key *key,
		int bank, struct agent_meta_store *store,
		uint64 expected_generation, uint64 expected_hash,
		int expected_migration)
{
	uint64 generation = 0;
	...
	if (key->resumable && probe.confirmed_bank == bank &&
		...
		return AGENT_META_BANK_VALID;
	status = agent_metadata_probe_read(
		key, bank, 1, store, &generation, &payload_hash, &migration, 0);
	...
	if (generation != expected_generation || payload_hash != expected_hash ||
...
uint64
agent_metadata_probe_epoch(void)
{
	return probe.epoch;
}

void
agent_metadata_probe_finish(uint64 epoch)
{
...
void
agent_metadata_probe_catalog_progress(int bank, uint offset)
{
...
void
agent_metadata_probe_progress(uint64 *sequence, int *bank, uint *phase,
			      uint *offset)
{
	if (sequence)
