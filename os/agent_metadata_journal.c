...
int
agent_meta_journal_plan_build(struct agent_meta_journal_plan *plan,
			      const struct agent_meta_journal_cursor *cursor,
			      uint scope_id,
			      struct workflow_lifecycle_key lifecycle,
			      const struct agent_meta_journal_change *changes,
			      uint count)
{
	uint blocks = agent_meta_journal_txn_blocks(count);
	uint slots;

	if (plan == 0 || cursor == 0 || changes == 0 || blocks == 0 ||
	    cursor->generation == ~0ULL ||
	...
	if (cursor->slots_used > AGENT_META_JOURNAL_SLOTS ||
	    slots > AGENT_META_JOURNAL_SLOTS - cursor->slots_used)
	...
	for (uint i = 0; i < count; i++) {
		amd_journal_slot *slot = &plan->slots[i];
		const struct agent_meta_record *record = &changes[i].record;

		if ((changes[i].operation != AGENT_META_JOURNAL_OP_UPSERT &&
		     changes[i].operation != AGENT_META_JOURNAL_OP_DELETE) ||
		...
		for (uint prior = 0; prior < i; prior++)
			if (changes[prior].record.slot == record->slot)
				return AGENT_META_JOURNAL_CORRUPT;
		journal_header_init(&slot->header,
			AGENT_META_JOURNAL_KIND_DATA, cursor, scope_id,
			lifecycle, i, count);
		slot->header.operation = changes[i].operation;
	...
	journal_header_init(&commit->header,
		AGENT_META_JOURNAL_KIND_COMMIT, cursor, scope_id, lifecycle,
		count, count);
	...
	if (slots != count + 1U) {
		...
		journal_header_init(&pad->header,
			AGENT_META_JOURNAL_KIND_PAD, cursor, scope_id, lifecycle,
			count + 1U, count);
...
static int
journal_diff_count(const struct agent_meta_store *base,
		   const struct agent_meta_store *next, uint scope_id,
		   uint *change_count)
{
	uint64 left = 0, right = 0;
	uint changes = 0;

	if (base == 0 || next == 0 || change_count == 0 ||
	    base->header.count > AGENT_FILE_META_MAX ||
	    next->header.count > AGENT_FILE_META_MAX ||
	    agent_durable_arena_validate(&base->durable) < 0 ||
	    agent_durable_arena_validate(&next->durable) < 0)
		return AGENT_META_JOURNAL_CORRUPT;
	for (uint offset = 0; offset < sizeof(base->durable);
	     offset += AGENT_META_JOURNAL_PATCH_DATA_BYTES) {
		uint bytes = sizeof(base->durable) - offset;

		if (bytes > AGENT_META_JOURNAL_PATCH_DATA_BYTES)
			bytes = AGENT_META_JOURNAL_PATCH_DATA_BYTES;
		if (memcmp((const uchar *)&base->durable + offset,
			   (const uchar *)&next->durable + offset, bytes) != 0)
			changes++;
		if (changes > AGENT_META_JOURNAL_MAX_DATA_RECORDS)
			return AGENT_META_JOURNAL_NO_SPACE;
	}
	while (left < base->header.count || right < next->header.count) {
		const struct agent_meta_record *old = left < base->header.count ?
			&base->records[left] : 0;
		const struct agent_meta_record *fresh = right < next->header.count ?
			&next->records[right] : 0;

		if (old != 0 && fresh != 0 && old->slot == fresh->slot) {
			if (memcmp(old, fresh, sizeof(*old)) != 0) {
				if (old->scope_id != scope_id ||
				    fresh->scope_id != scope_id)
					return AGENT_META_JOURNAL_CORRUPT;
				changes++;
			}
			left++;
			right++;
			   ...
			   (old != 0 && old->slot < fresh->slot)) {
			if (old->scope_id != scope_id)
				return AGENT_META_JOURNAL_CORRUPT;
			changes++;
			left++;
		} else {
			if (fresh->scope_id != scope_id)
				return AGENT_META_JOURNAL_CORRUPT;
			changes++;
			right++;
		}
	...
	*change_count = changes;
	return changes ? AGENT_META_JOURNAL_OK : AGENT_META_JOURNAL_INCOMPLETE;
}
...
int
agent_meta_journal_plan_diff(struct agent_meta_journal_plan *plan,
			     const struct agent_meta_journal_cursor *cursor,
			     const struct agent_meta_store *base,
			     const struct agent_meta_store *next, uint scope_id,
			     struct workflow_lifecycle_key lifecycle)
{
	uint64 left = 0, right = 0;
	...
	if (plan == 0 || cursor == 0 || !agent_scope_valid(scope_id) ||
	    base == 0 || next == 0 ||
	    !workflow_lifecycle_key_valid(lifecycle) ||
	    base->header.generation != cursor->generation ||
	    next->header.generation != cursor->generation + 1)
		return AGENT_META_JOURNAL_CORRUPT;
	status = journal_diff_count(base, next, scope_id, &count);
	if (status != AGENT_META_JOURNAL_OK)
		return status;
	blocks = agent_meta_journal_txn_blocks(count);
	slots = blocks * AGENT_META_JOURNAL_SLOTS_PER_BLOCK;
	if (blocks == 0 || cursor->slots_used %
			       AGENT_META_JOURNAL_SLOTS_PER_BLOCK != 0 ||
	...
	for (uint offset = 0; offset < sizeof(base->durable);
		...
		if (memcmp((const uchar *)&base->durable + offset,
		    ...
		    journal_diff_fill_patch(plan, out++, cursor, scope_id,
			    lifecycle, &base->durable, &next->durable,
			    offset, bytes, count) != AGENT_META_JOURNAL_OK)
			return AGENT_META_JOURNAL_CORRUPT;
	}
	while (left < base->header.count || right < next->header.count) {
		const struct agent_meta_record *old = left < base->header.count ?
		...
		if (old != 0 && fresh != 0 && old->slot == fresh->slot) {
			if (memcmp(old, fresh, sizeof(*old)) != 0 &&
			    journal_diff_fill_slot(plan, out++, cursor, scope_id,
				    lifecycle, AGENT_META_JOURNAL_OP_UPSERT,
				    fresh, count) != AGENT_META_JOURNAL_OK)
				return AGENT_META_JOURNAL_CORRUPT;
			left++;
			right++;
		} else if (fresh == 0 ||
			   (old != 0 && old->slot < fresh->slot)) {
			if (journal_diff_fill_slot(plan, out++, cursor, scope_id,
				    lifecycle, AGENT_META_JOURNAL_OP_DELETE,
				    old, count) != AGENT_META_JOURNAL_OK)
				return AGENT_META_JOURNAL_CORRUPT;
			left++;
		} else {
			if (journal_diff_fill_slot(plan, out++, cursor, scope_id,
	...
	if (slots != count + 1U) {
		...
		journal_header_init(&pad->header, AGENT_META_JOURNAL_KIND_PAD,
			cursor, scope_id, lifecycle, count + 1U, count);
		pad->header.previous_commit_hash = plan->commit_hash;
		pad->header.group_hash = commit->header.group_hash;
		pad->header.slot_hash = agent_meta_journal_slot_hash(pad);
	}
