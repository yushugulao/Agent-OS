...
require_text "host_tools/plain_ucore_reader.py" "require_pinned_output_root" "本地结果阅读器 does not pin its static output root"
require_text "host_tools/plain_ucore_reader.py" "reject_link_components\\(root / rel\\)" "本地结果阅读器 does not reject linked static path components"
require_text "host_tools/plain_ucore_reader.py" "file_path.relative_to\\(root\\)" "本地结果阅读器 does not keep static paths below its pinned root"
require_text "host_tools/plain_ucore_reader.py" "read_regular_file\\(path, maximum_bytes=READER_STATIC_MAX_BYTES\\)" "本地结果阅读器 does not use bounded descriptor-backed static reads"
require_text "host_tools/test_plain_ucore_reader.py" "rebound Reader output root was served" "本地结果阅读器 test does not reject output-root rebinding"
