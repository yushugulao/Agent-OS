...
for module in ${removed_metadata_modules}; do
	if [ -e "${ROOT_DIR}/os/${module}.c" ] ||
	   [ -e "${ROOT_DIR}/os/${module}.h" ]; then
		fail "retired metadata implementation remains in os/: ${module}"
	fi
done

for path in "${ROOT_DIR}"/os/agent*.c; do
	module="$(basename "${path}" .c)"
	if contains_word "${active_modules}" "${module}"; then
		continue
	fi
...
if grep -n -E 'RETIRED_METADATA_C_SRCS|agent_metadata_(journal|probe|recovery|scan|store|test)\.c' "${makefile}" >"${TMP_FILE}"; then
	fail "Makefile still carries the retired metadata source pipeline"
fi
: >"${TMP_FILE}"
...
if grep -n -E '#include[[:space:]]+"agent_metadata_(journal|probe|recovery|scan|store)' ${active_paths} >"${TMP_FILE}"; then
	fail "active owner includes a retired persistence header"
fi
: >"${TMP_FILE}"
if grep -n -E '\b(agent_metadata_store_|agent_metadata_scan_)[A-Za-z0-9_]*[[:space:]]*\(' ${active_paths} >"${TMP_FILE}"; then
