...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "stable") == 0) {
        ...
        if (status != HOST_SNAPSHOT_OK || snapshot.size != 14 ||
            ...
            host_snapshot_validate_path(argv[2], &snapshot, &host_error) !=
                HOST_SNAPSHOT_OK) return 1;
        host_snapshot_release(&snapshot);
        return 0;
    }
    ...
    if (strcmp(argv[1], "changed-path") == 0) {
        if (argc != 4) return 20;
        status = host_snapshot_read(argv[2], 0, &snapshot, &host_error);
        if (status != HOST_SNAPSHOT_OK || rename(argv[3], argv[2]) < 0)
            return 21;
        status = host_snapshot_validate_path(argv[2], &snapshot, &host_error);
        host_snapshot_release(&snapshot);
...
def compile_harness(
    directory: pathlib.Path, source_text: str, name: str
) -> pathlib.Path:
...
def run(executable: pathlib.Path, mode: str, *paths: pathlib.Path) -> int:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="mkfs-snapshot-") as raw_tmp:
        ...
        executable = compile_harness(tmp, source, "snapshot-test")
        for mode, paths in (
            ("stable", (stable,)),
            ("limit", (stable,)),
            ("empty", (empty,)),
            ...
            stable.write_bytes(b"stable-content")
            require(run(executable, mode, *paths) == 0,
                    f"host snapshot regression failed: {mode}")

        stable.write_bytes(b"stable-content")
        require(run(executable, "changed-content", stable) == 0,
                "post-snapshot content mutation was not rejected")
        stable.write_bytes(b"stable-content")
        replacement.write_bytes(b"replacement---")
        require(run(executable, "changed-path", stable, replacement) == 0,
...
\t    memcmp(first, second, snapshot->size) != 0) {"""
        ...
        mutant = compile_harness(tmp, stability_mutant, "snapshot-mutant-stable")
        stable.write_bytes(b"stable-content")
        require(run(mutant, "changed-read", stable) != 0,
        ...
        mutant = compile_harness(tmp, limit_mutant, "snapshot-mutant-limit")
        stable.write_bytes(b"stable-content")
        require(run(mutant, "limit", stable) != 0,
...
\t\t    memcmp(snapshot->data, current.data, snapshot->size) == 0;"""
        ...
        mutant = compile_harness(tmp, path_mutant, "snapshot-mutant-path")
        stable.write_bytes(b"stable-content")
        replacement.write_bytes(b"replacement---")
        require(run(mutant, "changed-path", stable, replacement) != 0,
                "mutation survived: replaced path was accepted")

    print("mkfs host snapshot tests: 10 regressions, 3 mutations PASS")
    return 0
