...
int main(void)
{
	...
	ok = ok && rp_file_contains("rp_backend", "reference_cases=7");
	ok = ok && rp_file_contains("rp_backend_exec", "runtime_cases=0");
	...
	ok = ok && rp_file_contains("rp_wfio", "package=workflow-portability");
	ok = ok && rp_file_contains("rp_web_bundle", "workflow_portability=rp_wfio");
	int backend_reference_cases = rp_get_int_value("rp_backend", "reference_cases=");
	int backend_runtime_cases = rp_get_int_value("rp_backend_exec", "runtime_cases=");
	int backend_reference_rows = rp_get_int_value("rp_backend_exec", "reference_case_rows=");
	ok = ok && require_equal("backend_reference_cases", backend_reference_cases, 7);
	ok = ok && require_equal("backend_runtime_cases", backend_runtime_cases, 0);
	ok = ok && require_equal("backend_reference_rows", backend_reference_rows, 7);
	...
	ok = ok && rp_file_contains("rp_backend_exec", "reference_case=plain-ucore;expected_input=rp_wfio;expected_artifact=rp_artifact_manifest;expected_outcome=native_programs_ok");
	ok = ok && rp_file_contains("rp_backend_exec", "reference_case=retry-recovery;expected_input=rp_retry_plan;expected_artifact=rp_stage_state;expected_outcome=recovered_align");
	ok = ok && rp_file_contains("rp_backend_exec", "reference_case=user-context;expected_input=rp_query;expected_artifact=rp_provpath;expected_outcome=user_space_context_log");
	ok = ok && rp_file_contains("rp_backend_exec", "reference_case=user-fsmeta;expected_input=rp_artifact_manifest;expected_artifact=rp_query;expected_outcome=file_manifest_scan");
	ok = ok && rp_file_contains("rp_backend_exec", "reference_case=user-recovery;expected_input=rp_retrylog;expected_artifact=rp_fix;expected_outcome=user_space_repair_record");
	ok = ok && rp_file_contains("rp_backend_exec", "reference_case=user-event;expected_input=rp_worker+rp_timeline;expected_artifact=rp_agent_run;expected_outcome=file_backed_event_log");
	ok = ok && rp_file_contains("rp_backend_exec", "reference_case=user-audit;expected_input=rp_audit+rp_provpath;expected_artifact=rp_package;expected_outcome=append_only_audit_files");
	ok = ok && rp_file_contains("rp_backend_exec", "reference_report_rows=7");
	ok = ok && rp_file_contains("rp_backend_exec", "runtime_pass_rows=0");
	ok = ok && rp_file_contains("rp_backend_exec", "performance_samples=0");
	ok = ok && !rp_file_contains("rp_backend_exec", "result=passed");
	ok = ok && !rp_file_contains("rp_backend_exec", "input_check=pass");
	ok = ok && !rp_file_contains("rp_backend_exec", "ticks=");
	...
	ok = ok && rp_file_contains("rp_study", "reference_metric=plain_ucore;expected_file_scans=128;expected_context_trusted=0;expected_rebuild_steps=6");
	ok = ok && rp_file_contains("rp_study", "reference_metric=agentos_ucore;expected_context_trusted=1;expected_batch_tools=1;expected_metadata_index=1");
	...
	if (!rp_append_file("rp_agentcmp", "evidence_role=demo_reference;catalog_generation=demo_expected;backend_reference_checks=21;reference_case_rows=7;runtime_pass_rows=0;performance_samples=0;plain_inputs=7;study_reference_metrics=2;reference_report_rows=7;backend_report_links=2;status=reference_ready")) return 1;
	...
	printf("rp_compare_plain: evidence_role=demo_reference catalog_generation=demo_expected status=reference_ready\n");
	return 0;
}
