...
int main(void)
{
	...
	ok = ok && rp_file_contains("rp_schedexec", "execution_records=2");
	...
	if (!rp_append_file("rp_agentcmp", "training_compliance_checks=92;requirements=4;training_records=4;competency=4;authorizations=3;gaps=1;open_gaps=0;charts=4;status=ready")) return 1;
